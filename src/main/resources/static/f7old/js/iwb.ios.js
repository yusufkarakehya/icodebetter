...
function onDeviceReady(){
	iwb.gcmId='dtGoRPDEjQk:APA91bE_7I-pnCHsCGdMA4nCXd83B7yaoS5Ald2XLDBhVEjL4r7ycc0NpQkyD35AKr6amtXyzLQcykPdGiLhXh_9QcDVqPchtSN2LXzxLZqG9Mkt3NgNW60-3gTf39xh8Bf9rJH8_OG5';
...
//		iwb.longPoll.init('../async/ajaxNotifications?&.w=' + iwb.webPageId, iwbLP);
	...
	/*	 var push = PushNotification.init({ "android": {"senderID": "553372575530"},  "ios": {"sound": true, "vibration": true, "badge": true}});
		 push.on('registration', function(data) {
			 ...
			 if(iwb.debug){console.log('iwb-registration');console.log(data.registrationId);}
		 });
		...
		iwb.app.addNotification({closeOnClick:true,title:'<b style="color:red;">Error</b>',message: 'onDeviceReady: ' + e, media:info4media(2)});
	}
}
...
//		if(iwb.debug){console.log('prepareMainMenu');console.log(d);}
		...
		var lmid = 'idx-logout-1';// + new Date().getTime();
//		j.data.push({icon:'logout',text:'Logout',href:'#',id:lmid});
		$$('#idx-main-menu').html(recMenu(j.data));
		if(!iwb.home){
			iwb.home = true;
			mainView.router.loadPage('showMList?_lid=15&_reload=1&.r='+Math.random());
		}
	}}); 
}
...
//						        console.log('Page Refreshed: ' + json8.dataUrl);
			...
			if(fp && fp.length){
				...
				fp.on('click', function (e) {
					var buttons=[];
					//if(navigator.camera)
					if(json4.pictureCount)buttons.push([{ text: 'Photo Gallery (' + (json4.pictureCount)+')' ,
		                   onClick: function () {
...
iwb.cleanZombiReq={
		timex:3000,
		start:function(){
			var j = iwb.cleanZombiReq;
			if(j.started)return;
			j.end();
			j.started=true;
			if(!j.id)j.id = setTimeout(j.doClean, 0);

		},
		end:function(){
			var j = iwb.cleanZombiReq;
			if(!j.started)return;
			if(iwb.debug)console.log('end-cleanZombiReq: ' + j.id);
			if(j.id){
				clearTimeout(j.id);
				j.id = false;
			}
		...
		doClean:function(){
			...
			j.id = false;
			var s='Z!';
			console.log('iwb.cleanZombiReq.doClean:start');
			if(j.started)try{
				var t=new Date().getTime(), f=0;
				for(var k in iwb.requests){
					var xhr = iwb.requests[k].xhr;
					var ntf=xhr.requestUrl.indexOf('ajaxNotification')>=0;
					console.log('iwb.requests['+k+'].xhr.readyState='+xhr.readyState + ' / ' + xhr.status + ' / ' + xhr.requestId + ' / ' + (t-iwb.requests[k].time) + ' / ' + xhr.requestUrl);
					s+=';'+xhr.readyState + '/' + xhr.status + '/' + (t-iwb.requests[k].time)+(ntf ? 'A':'');// + ' / ' + iwb.requests[k].xhr.requestUrl;
					if(xhr.readyState && xhr.readyState==4){
						s+='F';
						f++;						
					} else if((xhr.readyState && xhr.readyState==1 && ntf && (t-iwb.requests[k].time>190000)) || (!ntf && (t-iwb.requests[k].time>10000/* && xhr.requestUrl.indexOf('ajaxNotification')<0*/))){
						if(xhr.readyState && xhr.readyState==4){
							s+='F';
						} else {
							s+='***';
...
//						fireAjaxCallback('ajaxError ajax:error', , 'error', xhr, 'iwb-ios-timeout');
					}

/*					if((t-iwb.requests[k].time>10000) && (t-iwb.requests[k].time<15000)){ //mini timeout
						console.log('iwb.cleanZombiReq.doClean:xhr.abort()');
						console.log(iwb.requests[k].xhr);
					//	iwb.requests[k].xhr.abort()
					} */
//					console.log(iwb.requests[k].xhr);
				}
			}catch(e){console.log('doClean:exception for iwb.cleanZombiReq :'+e);}
			if(f)f=getActiveRequestCount(true);
			if(iwb.debug)console.log('iwb.cleanZombiReq.doClean('+f+'):end');
			s+='!Z'
			j.id = setTimeout(j.doClean, j.timex);
			if(iwb.iosDebug)iwb.app.addNotification({closeOnClick:true,title:'<b style="color:red;">Debug 4 CleanZombie</b>',message: s});
		}
}

//iwb.cleanZombiReq.start();
//iwb.longPoll.start();
