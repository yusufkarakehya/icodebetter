...
iwb.requestErrorHandler=function(obj){
	if(obj.errorType){
	    ...
	    case	'validation':
	    	...
	    	break;
	    default:
	    	iwb.notifyVue('error',obj.errorMsg || 'Unknown ERROR',obj.errorType+' Error');
	    }
	} else {
...
//		return h('img',{border:0,src:'../images/custom/'+(f ?'':'un')+'checked.gif'});
	return row[cell] && 1*row[cell] ? h('i',{class:'fa fa-check', style:{color:'green'}}) : null;
}
...
function fileAttachmentHtml(row,cell){//TODO
	return row[cell] && 1*row[cell] ? h('i',{class:'icon-paper-clip'}):null;
} 

function vcsHtml(row, cell){
	return row[cell] && 1*row[cell] ? h('i',{class:'icon-social-github'}):null;
}
...
iwb.getFieldRawValue=function(h, field, extraOptions){	
	if(!field || !field.value)return h('i',{style:"display:block;width:100%;color:gray;"},'(boş)');
	var options = extraOptions || field.options;
	if(!options || !options.length){
		var value = field.value;
		if(typeof value=='undefined' || value=='')return h('i',{style:"display:block;width:100%;color:gray;"},'(boş)');
		return h('div',{class:"el-input"},[h('b',{style:"font-weight:bold",class:'el-input__inner '},value)]);
	...
	if(value==undefined || value=='')return h('i',{style:"display:block;width:100%;color:gray;"},'(boş)');
	return h('div',{class:"el-input"},[h('b',{style:"font-weight:bold",class:'el-input__inner '},value)]);
}
...
var XGrid = Vue.component('x-grid', {
	...
	data(){
		...
		if(g.pageSize)columns.map(function(o){if(o.sortable)o.sortable='custom'});
		var f = false;
		columns.map(function(o){if(o.minWidth)f=!0;});
		if(!f){
			columns[columns.length-1].minWidth=columns[columns.length-1].width;
			delete columns[columns.length-1].width;
		}
		return {loading:true,
	        rows: [], sorting: [], totalCount: 0, columns:columns,
	 ...
	 methods: {
			...
			queryString() {
				let queryString = this.grid._url+buildParams2(this.grid.params, this.row);
				if(this.pageSize>0){
					queryString+='&limit='+this.pageSize+'&start='+(this.pageSize * this.currentPage);
				    const columnSorting = this.sorting[0];
				    if (columnSorting) {
				      const sortingDirectionString = columnSorting.direction === 'descending' ? ' desc' : '';
		...
		render(h){
	    	...
	    	return h('div',{},[
	   				h('el-table',{style:"width: 100%", props:{stripe:!0, data:this.rows},directives:[{name:'loading',value:this.loading}], on:{'row-dblclick':this.dblClick,'expand-change':this.expandChange, 'sort-change':this.sortChange}},
	   						columns.map(function(p){return h('el-table-column',{props:p})})),
	   				g.pageSize && h('el-row',{style:"padding-top: 10px"},[h('el-pagination',{on:{'current-change':this.currentPageChange},props:{background:!0,currentPage:this.currentPage+1, pageSize:g.pageSize,layout:'total, prev, pager, next', total:this.totalCount}})])
	        ]); 
		}
});
	...
	data(){
		...
		if(g.detailGrids){
			for(var qi=0;qi<g.detailGrids.length;qi++){
				if(!g.detailGrids[qi].grid.pk)g.detailGrids[qi].grid.pk=g.detailGrids[qi].pk||g.detailGrids[qi]._pk;
				if(!g.detailGrids[qi].grid.params)g.detailGrids[qi].grid.params=g.detailGrids[qi].params;
			}
			columns.unshift({type:'expand'});
		}
		...
		if(!f){
		 ...
		 return {loading:true,
		        rows: [], sorting: [], totalCount: 0, columns:columns,
		        lastQuery:'', currentPage:0, pageSize:g.pageSize, expandedRowIds:{}
		  }
	},
    methods: {
		onNewRecord(e,grid, row) {
		...
		queryString() {
			let queryString = this.grid._url+'&limit='+this.pageSize+'&start='+(this.pageSize * this.currentPage);
		    const columnSorting = this.sorting[0];
		    if (columnSorting) {
		      const sortingDirectionString = columnSorting.direction === 'descending' ? ' desc' : '';
		      queryString += '&sort='+columnSorting.columnName+sortingDirectionString;
...
//   			                  ,h('el-button',{props:{icon:"el-icon-menu", circle:!0}})]),
   				...
   				g.pageSize && h('el-row',{style:"padding-top: 10px"},[h('el-pagination',{class:'float-right', on:{'current-change':this.currentPageChange},props:{background:!0,currentPage:this.currentPage+1, pageSize:g.pageSize,layout:'total, prev, pager, next', total:this.totalCount}})])
        ]); 
	}
});
...
var XModal = Vue.component('modal', {
	render(h){
		return this.show && h(SlideYUpTransition,{props:{duration:500}},[
		    h('div',{class:"modal fade " +(this.show?'show d-block':'d-none')+(this.type === 'mini'?' modal-mini':'')+cls2str(this.modalClasses)
//		    	,role:'dialog',tabindex:'-1' TODO
		    	//,directives:[{name:'show',value:this.show}]
		    	,on:this.closableOutside ? {click:this.closeModal}:undefined
		    	},[
		    	  h('div',{class:"modal-dialog "+(this.type && this.type.length==2 ? 'modal-'+this.type:'')+ ' ' +(this.type === 'notice' ? 'modal-notice':'')},[
                      h('div',{on:{click:(e)=>{if(e && e.preventDefault)e.preventDefault();}},class:"modal-content"},[
                             this.header && h('div',{class:"modal-header"+cls2str(this.headerClasses)},[
	                              this.closable && h('button',{type:'button',class:"close",on:{click:this.closeModal}},[h('i',{class:"now-ui-icons ui-1_simple-remove"})])
	                              ,h('h3',{class:"modal-title"},[this.header,!this.viewMode && this.subHeader && h('span',this.subHeader),this.viewMode && h('el-button',{style:"position: relative;top: -3px;right: -12px;",props:{icon:"el-icon-edit", circle:!0},on:{click:this.toggleViewMode}}) ])
	                         ]),
//	                         this.header && h('hr',{style:'width:100%'}),
	                         h('div',{class:"modal-body"+cls2str(this.bodyClasses)},[
    ...
    ,components: {SlideYUpTransition}
  	,data(){
  		return {show:false,closable:true,closableOutside:true, type:'', header:false, subHeader:false, body:false, footer:false, footerClasses:'', modalClasses: '', headerClasses: '', bodyClasses:'', submit:false, viewMode:false};
  	},
    methods: {
    	toggleViewMode(e){
		...
		showModal(cfg){
			this.show=!0;this.type=cfg.type||'';this.submit=cfg.onSubmit||false;
			this.body=cfg.body; this.closable=typeof cfg.closable=='undefined'?true:!!cfg.closable;this.closableOutside=typeof cfg.closableOutside=='undefined'?true:!!cfg.closableOutside;
			this.modalClasses=cfg.modalClasses||'';this.headerClasses=cfg.headerClasses||'';this.bodyClasses=cfg.bodyClasses||'';this.footerClasses=cfg.footerClasses||'';
			this.header=cfg.header || false;this.subHeader= cfg.subHeader || false;this.footer=cfg.footer || false; this.viewMode = cfg.viewMode||false;
	  	}
    },
    watch: {
      show(val) {
    ...
    mounted(){
    	iwb.showModal = this.showModal;iwb.closeModal = this.closeModal;
    }
});
...
//					h("i", {class: "big-icon "+(q.icon || "icon-settings")}),
					h('card-block', {class: "pb-0"},[
						...
						h("div",{style:"color: #aaa;font-size: .9rem;margin-top:15px"},o.props.longDsc||"updated a minute ago")]
					)]);
		}
		name = name.name;
		var cmp=null;
		if(o.graph){
			return h('card', {class: "card-portlet shadow "+(o.props.color?'bg-'+o.props.color:'')}
			...
			return h('card', {class: "card-portlet shadow "+(o.props.color?'bg-'+o.props.color:''),style:o.props.height?"height:"+o.props.height:""}
			,[h("h3", { class: "form-header", style:"padding: 2px 2px 0px;;margin-bottom:0px " },name)
			  ,h("div", { style: "position: absolute;top: 20px;right: 20px;"},[h('i',{class:'now-ui-icons loader_refresh'})])
				,o.props.longDsc && h('p',{style:"color:gray"},o.props.longDsc)
				,h(XGrid,{props:o})]);
	...
	return {
		...
		render(h){
			...
			return h('div',{class:"container-fluid", style:""}, o.rows.map((rowItem)=>{
				return h('div',{class:"row", style:"padding-bottom: 30px;"}, rowItem.map((colItem)=> h('div',{class:props2css(colItem.props)}, [h(XPortlet,{props:{o:colItem}})])));//iwb.createPortlet(colItem)
			}));
		}
	}
}
