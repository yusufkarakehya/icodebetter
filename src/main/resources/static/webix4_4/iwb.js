...
function fmtTimeAgo(a){
	...
	if(a>60){
		a=Math.round(a/60);d=('minutes');
		if(a>60){
			a=Math.round(a/60);d='hours';
			if(a>24){
				a=Math.round(a/24);d=getLocMsg('days');
			}else if(a>15){
			...
			if(cfg.success)try{
				...
				if(j.success!==false){
            		...
            		try{
            			...
            			if(iwb.debug)iwb.confirm('iwb.request.error Expception. Throw?', function(){throw e});
            		}
...
function myHome(){
var gr1={x:'xi1-'+_page_tab_id,dash: {dashId:16,name:'Müşteriler', gridId:3262,tableId:2728,is3d:true,dtTip:0,graphTip:3,groupBy:'tbl.il',funcTip:0,funcFields:'-41', queryParams:{},legend:true}};
var gr2={x:'xi2-'+_page_tab_id,dash: {dashId:3,name:'Form by ObjectTip', gridId:21,tableId:40,is3d:false,dtTip:2,graphTip:1,groupBy:'tbl.object_tip',funcTip:1,funcFields:'731', queryParams:{"xform_id":"","xdsc":"","xmodule_id":"","xrender_tip":"","xobject_tip":"","xobject_id":""}}};
	...
	var dashLine={
		...
		css: "tiles",
		template: function(e) {
			for (var t = null, i = e.items, a = "<div class='flex_tmp'>", n = 0; n < i.length; n++) t = i[n], a += "<div class='item " + t.css + "'>", a += "<div class='webix_icon icon fa-" + t.icon + "'></div>", a += "<div class='details'><div class='value'>" + t.value + "</div><div class='text'>" + t.text + "</div></div>", a += "<div class='footer'>Daha fazla <span class='webix_icon fa-angle-double-right'></span></div>", a += "</div>";
			return a += "</div>"
		},
		data: {
			items: [{
	...
	var graphPie={type: "clean", height:350, css:'bg_raised', 
		rows: [{
			template: "<span class='webix_icon fa-pie-chart'></span>Müşterilerin İllere Göre Dağılımı",
			css: "sub_title",
			height: 32
		}, {template:'<div id="'+gr1.x+'" style="width:100%;height:100%"></div>', on:{onAfterRender:function(){iwb.dashboard(gr1.dash, gr1.x);}}}]
	};
	var graph1={
			type: "clean", height:250, css:'bg_raised', 
			rows: [{
				template: "<span class='webix_icon fa-sign-in'></span>Performans Değerlendirmesi",
				css: "sub_title",
				height: 30
			}, {
				view: "chart",
	...
	return {type: "clean",id:"contentKazim", 
		rows: [dashLine, {type:"clean", cols:[{gravity:3, type:"space", padding:10, rows:[graphPie,
		{rows:[{template:"<span class='webix_icon fa-tasks'></span>Bekleyen İşler",type:"header",css:"sub_title",height:50},{view:"list",css:"tasks_list",autoheight:!0,type:{marker:function(e){return"<span class='webix_icon_btn fa-bell-o marker "+e.type+"' style='max-width:32px;' ></span>"},check:webix.template('<span class="webix_icon_btn fa-{obj.$check?check-:}square-o list_icon" style="max-width:32px;"></span>'),template:function(e,t){return"<div class='"+(e.$check?"done":"")+"'>"+t.check(e,t)+"<span class='list_text'>"+e.text+"</span><span class='marker "+e.type+"'>"+(e.type||"")+"</span></div>"}},data:[{id:"1",text:"Prepare finance report"},{id:"2",text:"Solex project strategy  meeting",type:"projects"},{id:"3",text:"WestEurope partners call"},{id:"4",text:"Prepare presentation for summer conference",type:"company"},{id:"5",text:"Market research analysis"},{id:"6",text:"Check messages"},{id:"7",text:"Discussing new theme for website",type:"company"}],on:{onItemClick:function(e){var t=this.getItem(e);t.$check=!t.$check,this.refresh(e)}}},{css:"show_all bg",template:"Bütün işleri göster<span class='webix_icon fa-angle-double-right'></span>",height:40}]}
		
		]}, {gravity:2.3, type:"space", padding:10, rows:[{css:'bg_raised', height:400, view:"google-map",id:"map",key:"AIzaSyAi0oVNVO-e603aUY8SILdD4v9bVBkmiTg",zoom:5,center:[39.3671091,34.7545323]},graph1]}] }, {}]
	};
}
...
//    	console.log(e);
    ...
    folder: function(e) {
//    	console.log(e)
        return e.icon ? "<span class='webix_icon icon fa-"+(e.icon|| "cube")+"'></span>" : ""
    }
});
...
function fnRowDelete(g, pk){
	return function(){
		...
		console.log(sel);
		webix.confirm('Seçili kaydı Silmek istediğinden emin misiniz?',function(ax){
			if(ax)webix.ajax().post('ajaxPosstForm?a=3&_fid='+g.crudFormId+buildParams2(pk, sel), function(a,b,x){
...
function fnRowDelete2(xxx){
	webix.confirm('Silmek istediğinizden emin misiniz?',function(ax){
		if(ax)webix.ajax().post('ajaxPostForm?a=3&_fid='+xxx, function(a,b,x){
			webix.alert('Silindi');
		});
	});
}
...
function showFiles(t,k,z){
	var ee=webix.ui({
		view: "popup",
		autowidth: !0, position:z?undefined:'center',
		padding: 0,
//		css: "list_popup",
		body: {
			type: "clean",
			...
			rows: [{
				view: "list",
				autoheight: !0,
				url:'ajaxQueryData?_qid=806&_renderer=webix3_3&_tableId='+t+'&_tablePk='+k,
				type: {
					template: "<a href='dl/#value#?_fai=#id#' target=_blank><span class='webix_icon fa-download'></span>&nbsp;#value#</a>"
				}
			},{height:1,css:{"background-color":"lightgray"}}
		    ,{view: "uploader", label: "Dosya Yükle", upload:'upload2.form?profilePictureFlag=0&table_id='+t+'&table_pk='+k}
//			,{height:32,template: "<a href=# onclick='return attachFiles("+t+","+k+")'>&nbsp;Upload New Files ...</a>"}
			]
		}
	});
	if(z)ee.show(z); else ee.show();
	return false;
}

function attachFiles(t,k,z){
	var ee=webix.ui({
		view: "popup",position:z?undefined:'center',
		autowidth: !0, 
		padding: 0,
		body: {view: "uploader", label: "Dosya Yükle", upload:'upload2.form?profilePictureFlag=0&table_id='+t+'&table_pk='+k}
	});
	if(z)ee.show(z); else ee.show();
	return false;
}
...
function fileAttachmentHtmlOld(x,y,z, g){
	return x && 1*x ? ('<span style="cursor:pointer;" class="webix_icon fa-upload hi-highlight hi-highlight-orange" onClick="showFiles('+g.crudTableId+','+z.data[g.idField]+',this)"></span>'):'<span style="cursor:pointer;color:lightgray" class="webix_icon fa-download" onClick="attachFiles('+g.crudTableId+','+z.data[g.idField]+')"></span>';
}
function fileAttachmentHtml(x,y,z, g){
	return x && 1*x ? ('<span style="cursor:pointer;font-weight:bold;padding: 2px 4px;border-radius:50%;background-color:#969696;color:white;" class="status" onClick="showFiles('+g.crudTableId+','+z.data[g.idField]+',this)">&nbsp;'+x+'&nbsp;</span>'):'<span style="cursor:pointer;color:lightgray" class="webix_icon fa-upload" onClick="showFiles('+g.crudTableId+','+z.data[g.idField]+',this)"></span>';
}
...
function disabledCheckBoxHtml(f){
	return	function(x){
		return '<img src="../images/custom/'+(x && x[f] ?'':'un')+'checked.gif" border=0>';
	}
}
...
function fnRowEdit(g, pk){
	return function(ax){
		...
		if(href)mainPanel.loadTab({href:href,id:'crud1-f'+g.crudFormId+'-'+param});		
	}
}
...
//		g.on.onItemDblClick=function(ax){fnRowEdit2(g.crudFormId+buildParams2(pk, this.getItem(ax)));};
	...
	g.columns.push({header:' ', id: '_del', width: 50, template:function(ax){return '<span onclick="fnRowDelete2(\''+g.crudFormId+buildParams2(pk, ax)+'\')" style="cursor:pointer;" class="webix_icon fa-trash-o hi-highlight hi-highlight-red"></span>';}});
...
function fncBI(g,showMasterDetailReport){
	return function(){
		openPopup('showPage?_tid='+(g.crudTableId ? '1200&xtable_id=' + g.crudTableId:'2395&xquery_id='+g.queryId), '_blank', 1200, 800, 1);
	}
}
...
var subMenus1=[{id:10,value:"count"},{id:11,value:"sum"},{id:12,value:"avg"},{id:13,value:"max"},{id:14,value:"min"}];
...
//    buttons.push({view:"toolbar",css:"highlighted_header header6",paddingX:5,paddingY:5,height:40,cols:[{view:"pager",id:"pagerA",template:"{common.pages()}",autosize:!0,height:35,group:5}]});
	...
	buttons.push({view: "button", css: "button_danger", type: "iconButton",
		icon: "file-pdf-o",  width: 120, label: "Export" , click:function(){
			webix.toExcel($$(masterGrid.id));
		}
	});

	buttons.push({view: "button", css: "button_danger", type: "iconButton",
		icon: "area-chart",  width: 90, label: "BI" , click:fncBI(masterGrid,false)
	});
	...
	setTimeout(function(){
		...
		$$(masterGrid.id).$view.oncontextmenu = function(){return false};
		var cmenu=[{id:2,value:"Graphic"}];//,{id:5,value:"Group By"}
		if(masterGrid.$mapQueryField || masterGrid.$mapQueryFieldName)cmenu.unshift({id:6,value:"Map", submenu:subMenus1});
		
		webix.ui({
			  view:"contextmenu",id:"col-cmenu2"+masterGrid.id,width:250,data:cmenu
			  ,on:{
...
//					      masterGrid._stat=id-10;masterGrid._field=columns[selectedColumnIndex].id;
				      ...
				      switch(1*id){
					    case	2:
					    	  ...
					    	  webix.$$("group-"+masterGrid.id).setValue(columns[selectedColumnIndex].id);
					    	  $$("mymultiview"+masterGrid.id).setValue("idMapView"+masterGrid.id);
					    break;
	...
	if(masterGrid.$mapQueryField || masterGrid.$mapQueryFieldName){	
		buttons.push({view: "button", css: "button_primary", type: "iconButton",
			icon: "globe",  width: 95, label: "Map" , click:function(){webix.$$("aggr-"+masterGrid.id).setValue(10);webix.$$("graph-"+masterGrid.id).setValue(9);$$("mymultiview"+masterGrid.id).setValue("idMapView"+masterGrid.id);}
		});
	}
...
//	pageRows.push({view:"toolbar", id:"gt-"+masterGrid.id, css: "highlighted_header header2", height:33, cols:buttons});
	pageRows.push({type:"clean", id:"gt-"+masterGrid.id, height:42, cols:buttons});
	masterGrid.css='bg_raised';masterGrid.resizeColumn=!0;masterGrid.dragColumn=!0; 
...
//    pageRows.push({view:"toolbar",css:"highlighted_header header6",paddingX:5,paddingY:5,height:40,cols:[{view:"pager",id:"pagerA-"+masterGrid.id,template:"{common.pages()}",autosize:!0,height:35,group:5}]});
	...
	if(obj.detailGrids){
		for(var i=0;i<obj.detailGrids.length;i++){
			...
			dgr.resizeColumn=!0;dgr.dragColumn=!0;
			var r=[]
			var but2=prepareCrudButtons(dgr, dgr.pk || obj.detailGrids[i].pk);
			if(dgr.extraButtons)but2 = but2.concat(dgr.extraButtons);
                        but2.push({});
			but2.push({view: "button", css: "button_danger", type: "iconButton",
				icon: "file-pdf-o",  width: 120, label: "Export" , idx:dgr.id,click:function(aq){
//					console.log(this);console.log(aq);
			...
			but2.push({view: "button", css: "button_danger", type: "iconButton",
				icon: "area-chart",  width: 90, label: "BI" , click:fncBI(dgr,true)
			});
//			but2.push({}, { view:"button", type: "icon", icon: "wrench", value:"Info", width:30, align:"right" });
			
			r.push({type:"clean", padding:5, id:"gt-"+dgr.id, height:50,// css: "highlighted_header header5",
			    cols:but2});
			dgr._masterGrid=masterGrid;
			r.push(dgr);
			detailTabs.push({id: "dt-"+dgr.id, header:dgr.name, minWidth:250, body:{rows:r}});//width:200, 
		}
	...
	if(detailTabs.length>0){
		...
		pageRows.push({view:"tabview",css:'bg_raised', multiview:{ animate:{type:"flip", subtype:"vertical"}}, cells:detailTabs});
	...
	var graphOptions =[{id:1,value:"<span class='webix_icon fa-bar-chart'></span>Çubuk"},{id:3,value:"<span class='webix_icon fa-pie-chart'></span>Pasta"}];
	if(masterGrid.$mapQueryField || masterGrid.$mapQueryFieldName)graphOptions.push({id:9,value:"<span class='webix_icon fa-globe'></span>Harita"});
	var graphCombo={
      view: "combo",
      id: "graph-"+masterGrid.id,
      value: 3, width:200,
      options: graphOptions, placeholder:'Gösterim Şekli'
      ,on:{onChange:o=>{if(1*o!=9)webix.$$("group-"+masterGrid.id).show(); else webix.$$("group-"+masterGrid.id).hide();makeGraph(masterGrid);}}
    };
	var aggFields=[],grpFields=[];
	...
	var groupCombo={
		      ...
		      value: null, width:200,
		      options: grpFields, placeholder:'Gruplama Alanı'
		      ,on:{onChange:o=>makeGraph(masterGrid)}
		    };
	
	if(true || masterGrid.$mapQueryField)pageRows=[{
		view:"segmented", id:'tabbar'+masterGrid.id, value: 'idTableView'+masterGrid.id, multiview:true, hidden:true,options: [
			{ value: "<span class='webix_icon fa-table'></span><span style='padding-left: 4px'>TABLO</span>",  id: 'idTableView'+masterGrid.id},
			{ value: "<span class='webix_icon fa-globe'></span><span style='padding-left: 4px'>HARİTA</span>",  id: 'idMapView'+masterGrid.id}
		]}, {id:"mymultiview"+masterGrid.id,cells:[
			{id:"idTableView"+masterGrid.id, rows:pageRows},
			{id:"idMapView"+masterGrid.id, rows:[{type: "clean",height:40,cols:[
			    {view: "button", css: "button_primary", type: "iconButton", icon: "backward", label:'BACK',width: 100, click:function(){webix.$$("mymultiview"+masterGrid.id).setValue('idTableView'+masterGrid.id);}}
			    ,graphCombo,groupCombo,{},{view:'label',align:'right',width:150,label:'Aggregation'},{view:'combo',value:'10',width:120,id:'aggr-'+masterGrid.id,options:subMenus1, on:{onChange:o=>{if(1*o>10)webix.$$("qfield-"+masterGrid.id).show(); else webix.$$("qfield-"+masterGrid.id).hide();makeGraph(masterGrid); }}},{view:'combo',hidden:!0,on:{onChange:o=>{makeGraph(masterGrid);}}, placeholder:'İşlem Alanı', value:null,width:200,id:'qfield-'+masterGrid.id,options:aggFields}]},{type: "clean",/*on:{onAfterRender:makeTRMap}, */template:'<div id="idxMap2'+masterGrid.id+'" style="background-color:#b5b5b5;width:100%;height:100%"></div>'}]}
		], on:{onViewChange:function(prevID,nextID){if(nextID && nextID.indexOf("idMapView")==0)makeGraph(masterGrid); else masterGrid._lastGraph=false;}}}];
	return {id: obj.t, _lg:masterGrid.liveSync, type:'clean', header:masterGrid.name, close:true, minWidth:180, //width:180,
...
function makeGraph(_grid1){
	...
	function renderGraph(areaData){
		...
		if(tip==3){
			...
			AmCharts.makeChart("idxMap2"+_grid1.id, iwb.apply(extraCfg, {
				"type": "pie",
				"labelRadius": 15,
				"labelText": "[[value]]",
				"innerRadius": "50%",
	...
	var gx = $$(_grid1.id),grpMap={}, areaData=[];;
	...
	for(var k in grpMap){//0:count, 1:sum, 2.avg,3:max,4:min
		var val=0;
		switch(stat){
		case	0:val=grpMap[k].cnt;break;
		case	1:val=grpMap[k].sum;break;
		case	2:val=grpMap[k].cnt>0?grpMap[k].sum/grpMap[k].cnt:0;break;
		case	3:val=grpMap[k].min;break;
		case	4:val=grpMap[k].max;break;
		}
		areaData.push({
			"id": k,
			"value": val
...
function makeTRMap(_grid1){
	...
	if(_grid1.$mapQueryFieldName){
		var gx = $$(_grid1.id),ilMap={}, areaData=[];;
		if(gx.getFilter && _grid1.$filterField){
			var flt = gx.getFilter(_grid1.$filterField);
			if(flt)flt.setValue('');
		}
		...
		for(var k in ilMap){//0:count, 1:sum, 2.avg,3:max,4:min
			var val=0;
			switch(stat){
			case	0:val=ilMap[k].cnt;break;
			case	1:val=ilMap[k].sum;break;
			case	2:val=ilMap[k].cnt>0?ilMap[k].sum/ilMap[k].cnt:0;break;
			case	3:val=ilMap[k].min;break;
			case	4:val=ilMap[k].max;break;
			}
			areaData.push({
				"id": 'TR-'+k,
				"value": val
...
//	console.log(extDef);console.log(formDef);
            ...
            icon: "check",  width: 105, label: "SAVE" , click:function(){
            	...
            	webix.ajax().post("ajaxPostForm", p, function(text, xml, xhr){
            		...
            		if(r && r.success){
            			...
            			if(r.msgs)
            				webix.alert(r.msgs.join('<br/>'))
            			mainPanel.removeTab();
            		} else if(r && r.errorType=='validation'){
            			var msg='<b style="color:red">Doğrulama Hataları</b>';
            			if(r.errors)for(var qi=0;qi<r.errors.length;qi++){
            				msg+="<br/><li/>"+r.errors[qi].dsc+": <b>"+r.errors[qi].msg+'</b>';
            			}
            			webix.alert(msg);
            		}
            	});
	...
	if(formDef.a==1 && formDef.approval){
		if (formDef.approval.wait4start) {
			bar.push({view: "button", css: "button_success button_raised", type: "iconButton",
	            icon: "play-circle-o",  width: 140, label: "ONAY İSTE" , click:function(){
	            	approveTableRecord(901, formDef, formDef.approval.dynamic)
	            }});
	    } else {
	    	bar.push({view: "button", css: "button_success button_raised", type: "iconButton",
	            icon: "forward",  width: 140, label: "ONAYLA" , click:function(){
	            	approveTableRecord(1, formDef)
	            }});
	    	if (formDef.approval.returnFlag)bar.push({view: "button", css: "button_warning button_raised", type: "iconButton",
	            icon: "backward",  width: 140, label: "İADE ET" , click:function(){
	            	approveTableRecord(2, formDef)
	            }});
	    	bar.push({view: "button", css: "button_danger button_raised", type: "iconButton",
	            icon: "stop",  width: 130, label: "REDDET" , click:function(){
	            	approveTableRecord(3, formDef)
	            }});
	    	bar.push({view: "button", css: "button_primary", type: "iconButton",
	            icon: "flag-o",  width: 160, label: "ONAY LOGLARI" , click:function(){
	...
	if(formDef.fileAttachFlag)rows.push({view:"list",  id:"fl-"+formDef.id, type:"uploader",autoheight:true, borderless:true});
	var p={id: formDef.id, _l:formDef.liveSync, type:'clean', header:('<span class="webix_icon fa-'+(extDef.baseParams.a==2 ? 'plus':'edit') +'"></span>' )+formDef.name, close:true, minWidth:180, 
			body:{type:'wide',padding:10, rows:rows}};//width:180,
...
function promisLoadException2(e,x,w){
	console.log(e);console.log(x);
	webix.alert('Load ERROR:promisLoadException2');
}
var wndLogin=null;
function loginDlg(){
	if(wndLogin && wndLogin.isVisible())return;
	if(!wndLogin)wndLogin = webix.ui({
		  view:"window", height:250, head:'Session Expired',width:300,position:"center",modal:true,
		    body:{
	    view:"form", width:300,
	    elements:[
	      { view:"text", label:"User Name", value:_scd.userName, readonly:true},
	        { view:"text", id:"idLoginPassWord", type:"password", label:"Password"},
	        { margin:5, cols:[
	            { view:"button", value:"Login" , type:"form", click:function(){
	            	var ps=$$("idLoginPassWord").getValue();
	            	$$("idLoginPassWord").setValue("");
	            	if(ps){
	            		webix.ajax().post("ajaxAuthenticateUser", {userRoleId:_scd.userRoleId, locale:_scd.locale, customizationId:0, userName:_scd.userName, passWord:ps}, function(text, xml, xhr){
	                		var r=eval('('+text+')');
	                		if(r && r.success){
	            ...
	            { view:"button", value:"Vazgeç", click:function(){
...
function ajaxErrorHandler(e){
	if(e && e.errorType)switch(e.errorType){
	case	'session':
		loginDlg();
		break;
	case	'sql'://scripting
	...
	default:
		...
		webix.alert('no connection');
	
}

function gridQwRendererWithLink(field,tbl_id){
    return function(c){
        return c[field]!=undefined ? '<a href=# onclick="return fnTblRecEdit('+tbl_id+','+c[field]+')">'+c[field+'_qw_']+'</a>' :'';
    };
}

function gridUserRenderer(field){
    return function(c){
        return c[field]==_scd.userId ? c[field+'_qw_'] : '<a href=# onclick="return openChatWindow('+c[field]+',\''+c[field+'_qw_']+'\',true)">'+c[field+'_qw_']+'</a>';
    };
}
...
function approvalLog(r,z){
	...
	if(false && z){
    	...
    	bar.push({view: "button", css: "button_danger button_raised", type: "iconButton",
            icon: "stop",  width: 140, label: "REDDET" , click:function(){
            	alert('TODO');
            }});
    	bar.push({});
		rows.push({cols:bar});
	...
	rows.push({
		view: "list",
		autoheight: !0, css: "chat_list",
		maxHeight: 500, position:z?undefined:'center',
...
//				var text = "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae. ";
				...
				html += "<div class='text'><div class='name'>"+obj.user_id_qw_+"<div class='time'><b>"+obj.approval_action_tip_qw_+'</b> '+obj.log_dttm+"</div></div>";
				html += (obj.dsc||'...')+"</div>";
				return html;
			}
		}
	...
	var ee=webix.ui({
		view: "popup",
		width: 500, position:z?undefined:'center',
		padding: 0,
//		css: "list_popup",
		body: {
			type: "clean",
...
function approvalHtml(x,y,z){
	...
	if(!z || !z.data)return '';
	z=z.data;
	var str='<a href=# onclick="return approvalLog('+z.pkpkpk_arf_id+',this)"';
	...
	str+='>';
	if(x>0)str+='<span style="color:red;padding:0;" class="webix_icon fa-flag-o"></span>';
...
iwb.openPopup=function(url,name,x,y,r){
	...
	if(!wh) webix.alert(getLocMsg('js_once_popup_engel_kaldir')); else wh.focus();
	return false;
}
...
//	console.log(s);console.log(d);console.log(p);
	if(typeof s.hiddenValue!='undefined' || typeof d.hiddenValue!='undefined'){
...
//	console.log(dd);
	...
	if(ss.getValue() && dd.getPopup() && dd.getPopup().getList()){
		var zz=dd.getPopup().getList();
		zz.clearAll();
		zz.load('ajaxQueryData?'+iwb.map2str(p(ss.getValue()))+'&_renderer=webix3_3');
	}
}
...
iwb.loadCombo=function(s,p,formAction){
	...
	if(ss){
		...
		s.on={onAfterRender:function(a,b,c){
			if(this.getPopup() && this.getPopup().getList()){
				this.getPopup().getList().load(url);
			}
		}};
	}
}
...
//	var dg=aq._dg, gid=aq._gid;
	...
	iwb.request({url:'ajaxQueryData4StatTree?_gid='+dg.gridId+'&_stat='+dg.funcTip+'&_qfid='+dg.groupBy+'&_dtt='+dg.dtTip, params:iwb.apply(params,dg.queryParams), successCallback:function(az){
		...
		case	6://stacked area
			...
			AmCharts.makeChart(gid, iwb.apply(extraCfg,{
			      "type": "serial",
