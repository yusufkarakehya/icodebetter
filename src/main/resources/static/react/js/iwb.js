...
iwb.JSON2URI=function(j){
	if(!j)return '';
	var s=''
	for (key in j)s += encodeURIComponent(key)+"="+encodeURIComponent(j[key])+"&";
	return s;
...
//	console.log('callAttributes',callAttributes);
	...
	} else return [/*_(CardHeader, {className:'xform'},_('span',{style:{fontSize:'1.2em'}},getForm.name)
				,_("div", {className: "card-actions"}
					, getForm.commentFlag && _("a", {href: "#"}, _("i", {className: "icon-bubble"}
						, getForm.commentCount>0 && _(Badge,{pill: !0,color: "danger",style:{marginLeft:'-8px', position: 'absolute'},className: "float-right"},""+getForm.commentCount)))
		        , getForm.fileAttachFlag && _("a", {href: "#"}, _("i", {className: "icon-paper-clip"}
		        	, getForm.fileAttachCount>0 && _(Badge,{pill: !0,color: "danger",style:{marginLeft:'-8px', position: 'absolute'},className: "float-right"},""+getForm.fileAttachCount))))
		)*/
	  ...
	  constructor(props) {
		    ...
		    if(this.props.searchForm){//hidden:!!this.props.grid.globalSearch
		    	...
		    	this.searchForm = _(Nav, {style:{}},_('div',{className:'hr-text'},_('h6',null,'Arama Kriterleri'))
			    	,_('div',{style:{zoom:'.9'}},this.props.searchForm.getExtDef({callback:false},this))
			    	,_('div',{style:{height:'10px'}}),_('div',{className:'hr-text'},_('h6',null,'Şablonlar'))
			    	,_(Link,{style:{padding:'2px'},to:''},_('i',{className:'icon-star'}),' ',' Yıllık Faturalar')
			    	...
			    	,this.props.detailGrids && this.props.detailGrids.length>1 && this.props.detailGrids.map(function(a,key){
			    		return _('div',{key:key,style:{padding: "3px 0px 2px 3px", color: "#6d7284", fontSize:".9rem"}},a.grid.name,_("label",{ "class": "float-right switch switch-xs switch-3d switch-"+dgColors[key % dgColors.length]+" form-control-label" },
			    				  ...
			    				  _("span", { "class": "switch-label" }),
			    				  _("span", { "class": "switch-handle" })
			    				));
			    	})
		    	);
		    }
...
//			if(this.state.loading===true)return;
		  ...
		  //    this.setState({ loading: false });
		    ...
		    this.setState({loading: true});
		    var params= Object.assign({},params||{},this.props.searchForm ? iwb.getFormValues(document.getElementById(this.props.searchForm.id)):{});
//		    console.log(params);
		      ...
		      .then(
		        (result) => {
		        	if(result){
		      			...
		      			if(r){
		      		    	var state = this.state;
		      		    	tabs.push({k:action, i:action==='2' ? "icon-plus":"icon-pencil", title:action==='2' ? " Yeni":" Düzenle",  v:r});
