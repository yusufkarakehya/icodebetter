...
if(!Ext.ux)Ext.ux={};
try{
if(Ext.ux.TabCloseMenu)Ext.override(Ext.ux.TabCloseMenu,{
	createMenu : function(){	   
      //  if(!this.menu){
            var items = [{
                itemId: 'close',
            ...
            if(_scd.customizationId==0/* && this.tabs.getActiveTab()._callCfg.request._tid*/){
                ...
                items.push({
                    ...
                    handler:function(ax,bx,cx){
        				...
        				if(templateId)promisRequest({
	        				url: 'ajaxQueryData?_qid=1745',
	        				params: {xtable_id: 64, xtable_pk: templateId, xlocale:_scd.locale},
	        				successCallback: function (j) {
	        					var str= (j.data && j.data.length) ? ('a=1&tdoc_id='+j.data[0].doc_id): ('a=2&table_id=64&table_pk='+templateId);
	        					mainPanel.loadTab({
	        						attributes: {
	        							_title_: 'Doc - ',
	        							modalWindow: true,
            ...
            if (this.tabs.getActiveTab()._myPage){
            	...
            	items.push({
                    ...
                    handler: function(){            		
	            		...
	            		promisRequest({url:'ajaxPostForm?_fid=1565&a=1&portlet_grids_orders='+tmp_str, successCallback:function(j){
	            			Ext.Msg.alert(getLocMsg('js_bilgi'),getLocMsg('js_islem_basariyla_tamamlandi'));
	            		}});	            		           		
            	}
                });
            }
...
}catch(eq){console.log(eq);}
//GridPanel
if(!Ext.ux.tree)Ext.ux.tree={};

try{
if(Ext.ux.tree.CheckTreePanel)Ext.override(Ext.ux.tree.CheckTreePanel, {
	getPath : function(id){
...
if(Ext.ux.form.SuperBoxSelect)Ext.override(Ext.ux.form.SuperBoxSelect, doQueryExtended);
}catch(eq){console.log(eq);}
...
try{
if(Ext.ux.form && Ext.ux.form.LovCombo)Ext.ux.form.DialogLovCombo = Ext.extend(Ext.ux.form.LovCombo, {
	initComponent: function() {
...
try{
//--- A ComboBox with a secondary trigger button that clears the contents of the ComboBox
if(Ext.ux.form.LovCombo)Ext.ux.form.AddLovCombo = Ext.extend(Ext.ux.form.LovCombo, {
	initComponent: function() {
		// template with checkbox
		if(!this.tpl) {
			this.tpl = 
...
Ext.form.LocaleMsgKey = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onTrigger2Click : function(){
        ...
        if(v)
        	mainPanel.loadTab({attributes:{modalWindow:true, href:'showFormByQuery?_fid=1627&_qid=1648&xlocale_msg_key='+v}})
		this.focus();
    }
});
}catch(eq){console.log(eq);}
...
Ext.ux.TreeCombo = Ext.extend(Ext.form.TriggerField, {
    ...
    onRender : function(ct, position){
        ...
        if(this.hiddenName){
            ...
            if(typeof this.value!='undefined')this.hiddenField.value = this.value;
        }
    ...
    initComponent : function(){
		...
		if(this.allowBlank){
			this.triggerConfig.cn.push({tag: "img", src: Ext.BLANK_IMAGE_URL, cls: "x-form-trigger x-form-clear-trigger"});
		}
            ...
            if(!n){
            	...
            	for(var qi in this.treePanel.nodeHash){
            		if(this.treePanel.nodeHash[qi].attributes.children){
            			if(findSubNode(this.treePanel.nodeHash[qi].attributes.children,v)){
            				break;
            			}
            		}
            	}
            	if(resx.length){
                	var txt='';
            		for(var qi=0;qi<resx.length;qi++)txt='\\' + resx[qi].text+ txt;
            	...
            	if(!this.hideParentNodes){
            		while(n && n.getRootNode){
            			n=n.getRootNode();
            			if(n.text)txt=n.text + '\\' + txt;else break;
            		}
            	}
    ...
    onTreeNodeClick: function(node, e) {
    	...
    	if(!this.hideParentNodes){
    		var n=node;
    		while(n=n.parentNode)if(n.text)txt=n.text + '\\' + txt;else break;
    	}
        this.setRawValue(txt);
        this.value = node.id;
        if(this.hiddenField){
...
Ext.form.FormCellCodeDetail = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    setValueFromSystem: function(e){
        ...
        if(!parentForm){
        	Ext.Msg.alert(getLocMsg('js_bilgi'),'Muhtemelen Ayni SiraNo\'ya sahip oldugu icin aclmiyor');
        }else if((typeof(e) != 'undefined' && e.ctrlKey) || !this.getValue() || parentForm.baseParams.a!=1){
        	var xthis = this;
        	var params2=parentForm.getForm().getValues();//Ext.apply(parentForm.baseParams||{},parentForm.getForm().getValues());
        	var qq={}; if(parentForm.baseParams)qq=parentForm.baseParams;
        	for(var ki in qq)if(typeof params2[ki]=='undefined')params2[ki]=qq[ki];
        	promisRequest({url:'ajaxFormCellCode?_formCellId='+this.formCellId,params:params2,
        		successCallback:function(json){
        			if(json.msg)
    	        		Ext.infoMsg.msg(json.info||'hata',json.msg,json.pauseTime||3);
    ...
    onTrigger2Click : function(){
        ...
        if(!parentForm){
       	...
       	mainPanel.loadTab({attributes:{modalWindow:true, title:getLocMsg('js_coding')+': '+this.fieldLabel,href:'showFormByQuery?_fid=1011&_qid=1033&xform_cell_id='+this.formCellId,_parentFormCell:this,_parentForm:parentForm}});
    }
});
}catch(eq){console.log(eq);}
...
Ext.override(Ext.form.BasicForm, {
    findInvalid: function() {
        ...
        if(l>0)Ext.Msg.show({title: getLocMsg('js_alan_hatali_girilmistir'), msg: result, icon: Ext.MessageBox.ERROR});
    }
});
}catch(eq){console.log(eq);}
...
try{
if(Ext.ux.form.SuperBoxSelect)Ext.override(Ext.ux.form.SuperBoxSelect, {
	   assertValue:null,
...
Ext.infoMsg = {
		...
		prompt:function(msg,defaultValue, callback,type){
			...
			var c={text: msg+'<br/><br/><input autofocus value="'+defaultValue+'" id="'+idx+'">',theme:'metroui', type: type||'info', modal:!0,layout: 'center', animation:{open:null,close:null},closeWith:[],
        ...
        msg : function(type, msg, timeout){
        	...
        	new Noty({
        	    ...
        	    layout: pos+'Right',timeout:1000*(timeout||3),
        	    animation: {
        ...
        wow : function(type, msg, timeout){
        	...
        	new Noty({
        	    text: msg,theme:'metroui', type: type,
        	    layout: 'topCenter',timeout:1000*(timeout||10),
        	    animation: {
	...
	Ext.util.Format.getRecordTree=function(rrr){
	    ...
	    if(rrr._record && rrr._record.length){
	        ...
	        for(var qi=rs.length-1;qi>=0;qi--){
	                ...
	                var rdsc=r.dsc; 
	                if(!rdsc)rdsc='(...)';else if(rdsc.length>200)rdsc=rdsc.substring(0,197)+'...';
	                ss+=(qi!=0 ? ': <a href=# onclick="return fnTblRecEdit('+r.tid+','+r.tpk+');">'+rdsc+'</a>':': <b><a href=# onclick="return fnTblRecEdit('+r.tid+','+r.tpk+');">'+rdsc+'</a></b>');// else ss+=': (...)';
	        }
	        if(ss)s+='<div class="dfeed">'+ss+'</div>';
	    }
	    if(rrr.insert_dttm)s+='<span class="cfeed"> · '+fmtDateTimeWithDay2(rrr.insert_dttm)+'</span> · ' + ('<a href=# onclick="return fnTblRecComment('+rrr.tid+','+rrr.tpk+');">'+(!rrr.tcc ? 'Yorum Yap':((rrr.comments && rrr.comments.length<rrr.tcc?'Bütün ':'') +'Yorumlar ('+rrr.tcc+')'))+'</a>');
	    return s;
	}
}catch(e){if(1*_app.debug != 0 && confirm('ERROR Ext.util.Format definitions!!! throw?'))throw e;}
