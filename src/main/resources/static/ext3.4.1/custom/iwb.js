...
function openPopup(url,name,x,y,r){
	...
	if(!wh) Ext.infoMsg.alert('info', getLocMsg('js_once_popup_engel_kaldir')); else wh.focus();
	return false;
}
...
function showTableChildList(e, vtip, vxid, mtid, mtpk, relId){
	...
	promisRequest({url:'ajaxGetTableRelationData',params:{_tb_id:mtid, _tb_pk:mtpk, _rel_id:relId}, successCallback:function(j){
			...
			if(j.browseInfo.totalCount>j.browseInfo.fetchCount){
				items.push('-');
				items.push({text:getLocMsg('js_more')+'....(Total '+j.browseInfo.totalCount+')', handler:function(){Ext.infoMsg.alert('info','soooon')}});// TODO
			}
...
//						recordInfoWindow.destroy();
						...
						recordInfoWindow = new Ext.Window({
			        		...
			        		width: 500, 
			        		autoHeight:true, bodyStyle:'padding:3px;',
...
function addDefaultSpecialButtons(xbuttons, xgrid){
    ...
    if((_app.form_conversion_flag && 1*_app.form_conversion_flag && xgrid.formConversionList) || (_app.mail_flag && 1*_app.mail_flag && xgrid.formSmsMailList)){
        	...
        	for(var qz=0;qz<xgrid.formConversionList.length;qz++){
				...
				xgrid.formConversionList[qz].handler=function(aq,bq,cq){
					...
					if(aq.preview){
						...
						promisRequest({url:'ajaxPostEditGrid',params:pr, requestWaitMsg:true, successCallback:function(j){Ext.infoMsg.alert('info',j.msgs.join('<br>'));}}); 
					}
				}
        	}
	...
	xbuttons.push({id:'grd_pers_buttons'+xgrid.gridId, text:getLocMsg('js_kisisellestir'), _grid:xgrid,
		menu:{items:[{text:getLocMsg('js_bu_ayarlari_kaydet'), iconCls:'icon-ekle',_grid:xgrid,handler:function(ax,bx,cx){
				...
				promisRequest({requestWaitMsg:true,url:'ajaxExecDbFunc?_did=648',params:params, successCallback:function(j){
					Ext.infoMsg.alert('success', getLocMsg('js_mazgal_yeni_ayarlarla_gorunecek'));
				}});
				}
...
function addDefaultPrivilegeButtons(xbuttons, xgrid){
	if(_scd.administratorFlag || _scd.customizationId==0){
		...
		xbuttons.push({tooltip:getLocMsg('js_ayarlar'),cls:'x-btn-icon x-grid-setting', _activeOnSelection:false, _grid:xgrid,menu: xxmenu});
	}
}
...
//    addHelpButton(buttons, mainGrid, 64, mainGrid.extraOutMap.tplId);    
	if(false && mainGrid.gridReport)addDefaultReportButtons(buttons,mainGrid);
...
//		    addHelpButton(buttons, detailGrid, 5, detailGrid.gridId);
	     	if(false && detailGrid.gridReport)addDefaultReportButtons(buttons,detailGrid, true);
...
//	        	if(typeof onlineUsersGridPanel!='undefined' && onlineUsersGridPanel)reloadOnlineUsers();
	            ...
	            Ext.infoMsg.alert('error',resp.errorMsg||getLocMsg('js_yanlis_kullanici_adi_sifre'));
			}
    	},
	    failure: function(o,resp){
	        ...
	        if(resp.errorMsg){
	            ...
	            Ext.infoMsg.alert('error',resp.error||getLocMsg('js_verileri_kontrol'),'error');
	        }
	    }
    });
  return false;    
...
//	    		Ext.Msg.hide();
			...
			if(rcfg.successResponse)
				...
				} else Ext.infoMsg.alert('Error','Framework Error(Ajax.Request)','error'); //???
			}
		},
...
function approveTableRecord(aa,a){
    ...
    if(aa==2 && 1*sel.data.return_flag==0){
    	Ext.infoMsg.alert('info',getLocMsg('js_bu_surecte_iade_yapilamaz'),'info');
    	return
    }     
		...
		buttons: [{text:getLocMsg('js_tamam'),
				handler: function(ax,bx,cx){	
					...
					promisRequest({
						...
						successDs: a._grid.ds
						,successCallback:aa!=901?win.close():function(){win.close();Ext.infoMsg.alert('info',getLocMsg('js_onay_sureci_baslamistir'),'info');}
					});
				}},
...
//							     "legend":{"position":"left"}, //","marginRight":100,"autoMargins":false
			...
			if(false && detailGrid.gridReport)addDefaultReportButtons(buttons, detailGrid);
			addDefaultPrivilegeButtons(buttons,detailGrid);
...
function prepareDetailGridCRUDButtons(grid,pk,toExtraButtonsFlag){
	...
	if(buttons.length>0){
		if(toExtraButtonsFlag){
			...
			add_menu();
			if(false && grid.gridReport)addDefaultReportButtons(buttons, grid);
...
function getUsers4Chat(users, pix, onlineStatus){
	...
	var	str='';
	for(var qi=0;qi<users.length;qi++)str+=', &nbsp;'+(pix ? '<img src="sf/pic'+users[qi].userId+'.png" class="ppic-mini" style="margin-top: -2px;'+(onlineStatus ? 'border:3px solid '+(usersBorderChat[qi % usersBorderChat.length]):(users[qi].userDsc.endsWith("·")?'border:3px solid #37cc00':''))+';"> ':'')+'<a href=# onclick="return openChatWindow('+users[qi].userId+',\''+users[qi].userDsc+'\',true)"><span style="color: #ced5d8;">'+users[qi].userDsc+'</span></a>';
