...
public class PostgreSQL extends BaseDAO {
  ...
  public Object getCustomizedObject(String hql, int objectId, Object tenantId, String onErrorMsg) {
    ...
    if (list.size() == 0) {
      ...
      throw new IWBException(
          "framework", onErrorMsg, objectId, null, "Wrong ID: " + onErrorMsg, null);
  ...
  private boolean loadForm(W5FormResult f) {
    ...
    f.getForm()
        .set_formCells(
            find(
                "from W5FormCell t where t.formId=? AND t.projectUuid=? order by t.tabOrder, t.xOrder, t.dsc",
                f.getFormId(),
                projectId));
        ...
        case 31: // code_list
          if (GenericUtil.uInt(fc.getLookupIncludedParams()) == 0) {
            ...
            fc.set_formCellCodeDetailList(
                find(
                    "from W5FormCellCodeDetail t where t.formCellId=? AND t.projectUuid=? order by t.tabOrder",
                    fc.getFormCellId(),
                    projectId));
          break;
      }
    if (form.getRenderTip() != 0) { // eger baska turlu render edilecekse
      form.set_moduleList(
          find(
              "from W5FormModule t where t.formId=? AND t.projectUuid=? order by t.tabOrder",
              form.getFormId(),
              projectId));
    }
    ...
    f.getForm()
        .set_toolbarItemList(
            find(
                "from W5ObjectToolbarItem t where t.objectTip=? AND t.objectId=? AND t.projectUuid=? order by t.tabOrder",
                (short) 15,
                f.getFormId(),
                projectId));
    f.getForm()
        .set_formHintList(
            find(
                "from W5FormHint h where h.activeFlag=1 AND h.formId=? AND h.projectUuid=? order by h.tabOrder",
                f.getFormId(),
                projectId));
    if (f.getForm().get_formHintList().isEmpty()) f.getForm().set_formHintList(null);

    if (f.getForm().getObjectTip() != 1
        && f.getForm().getRenderTemplateId() != 0) { // grid(seachForm) degilse ve templateId varsa
      W5Page template =
          (W5Page)
              getCustomizedObject(
                  "from W5Page t where t.templateId=? and t.projectUuid=?",
                  f.getForm().getRenderTemplateId(),
                  projectId,
                  null); // ozel bir client icin varsa
    ...
    switch (f.getForm().getObjectTip()) {
      ...
      case 2: // table icin ise
        ...
        if ((fieldMap1.get("INSERT_USER_ID") != null || fieldMap1.get("insert_user_id") != null)
            && (fieldMap1.get("VERSION_USER_ID") != null
                || fieldMap1.get("version_user_id") != null)) {
        ...
        if (FrameworkSetting.sms || FrameworkSetting.mail) {
          f.getForm()
              .set_formSmsMailList(
                  find(
                      "from W5FormSmsMail t where t.projectUuid=? AND t.formId=? AND t.activeFlag=1 order by t.smsMailSentTip,t.tabOrder",
                      projectId,
                      f.getFormId()));
          if (f.getForm().get_formSmsMailList().isEmpty()) f.getForm().set_formSmsMailList(null);
            ...
            for (W5FormSmsMail fsm : f.getForm().get_formSmsMailList()) {
              //						if(fsm.getSmsMailTip()==0)f.getForm().set_mailFlag(true); else
        ...
        for (W5Conversion cnv : f.getForm().get_conversionList()) {
          cnv.set_conversionColList(
              find(
                  "from W5ConversionCol t where t.projectUuid=? AND t.conversionId=? order by t.tabOrder",
                  projectId,
                  cnv.getConversionId()));
      ...
      case 1: // grid icin ise
        ...
        query.set_queryParams(
            find(
                "from W5QueryParam t where t.projectUuid=? AND t.queryId=? order by t.tabOrder",
                projectId,
                queryId));
      ...
      case 4: // db func
        ...
        if (dbf == null) {
          dbf =
              (W5GlobalFunc)
                  getCustomizedObject(
                      "from W5GlobalFunc t where t.dbFuncId=? AND t.projectUuid=?",
                      f.getForm().getObjectId(),
                      projectId,
                      "GlobalFunc");
          dbf.set_dbFuncParamList(
              (List<W5GlobalFuncParam>)
                  find(
  ...
  */
  ...
  public W5QueryResult getQueryResult(Map<String, Object> scd, int queryId) {
    ...
    if (queryResult.getQuery() == null) {
      W5Query query =
          (W5Query)
              find(
                      "from W5Query t where t.queryId=? AND t.projectUuid=?",
                      queryResult.getQueryId(),
                      projectId)
                  .get(0); // ozel bir client icin varsa
      queryResult.setQuery(query);
      queryResult
          .getQuery()
          .set_queryFields(
              find(
                  "from W5QueryField t where t.queryId=? AND t.tabOrder>0 AND t.postProcessTip!=99 AND t.projectUuid=? order by t.tabOrder",
                  queryResult.getQueryId(),
                  projectId));
      queryResult
          .getQuery()
          .set_queryParams(
              find(
                  "from W5QueryParam t where t.queryId=? AND t.projectUuid=? order by t.tabOrder",
                  queryResult.getQueryId(),
                  projectId));
    ...
    switch (queryResult.getQuery().getQuerySourceTip()) {
      ...
      case 15:
        if (queryResult.getQuery().getMainTableId() != 0) {
          queryResult.setMainTable(
              FrameworkCache.getTable(projectId, queryResult.getQuery().getMainTableId()));
        }
    }
    return queryResult;
  }

  private void prepareLookupTableQuery(
      W5QueryResult queryResult, StringBuilder sql2, AtomicInteger paramIndex) { // TODO
    List<Object> preParams = new ArrayList<Object>();
    for (W5QueryField qf : queryResult.getQuery().get_queryFields())
      if (qf.getPostProcessTip() == 12
          && qf.getLookupQueryId() != 0
          && (queryResult.getQueryColMap() == null
              || queryResult
                  .getQueryColMap()
                  .containsKey(qf.getDsc()))) { // queryField'da postProcessTip=lookupTable olanlar
        // saptaniyor
          ...
          if (tqf.getSummaryRecordSql().contains("${")) {
            Object[] oz =
                DBUtil.filterExt4SQL(
                    tqf.getSummaryRecordSql(),
                    queryResult.getScd(),
                    queryResult.getRequestParams(),
                    null);
            sql2.append(oz[0]);
            if (oz[1] != null) preParams.addAll((List) oz[1]);
          } else sql2.append(tqf.getSummaryRecordSql());
      ...
      } else if (qf.getPostProcessTip() == 13
          && qf.getLookupQueryId() != 0
          && (queryResult.getQueryColMap() == null
              || queryResult
                  .getQueryColMap()
        ...
        if (tqf != null && tqf.getSummaryRecordSql() != null) {
          ...
          if (tqf.getSummaryRecordSql().contains("${")) {
            ...
            if (oz[1] != null) preParams.addAll((List) oz[1]);
          } else sql2.append("string_agg(").append(tqf.getSummaryRecordSql()).append(",',')");

          sql2.append(" from ")
              .append(tqf.getDsc())
              .append(" x where x.")
              .append(tqf.get_tableParamList().get(0).getExpressionDsc())
              .append(" in (select q.satir::int from iwb.tool_parse_numbers(z.")
              .append(qf.getDsc())
              .append(",',') q)");
  ...
  public void runQuery(W5QueryResult queryResult) {
    ...
    try {
              ...
              new Work() {
                ...
                public void execute(Connection conn) throws SQLException {
                  ...
                  if (queryResult.getFetchRowCount() != 0) {
                    if (false && !GenericUtil.isEmpty(queryResult.getExecutedSqlFrom())) {
                      s =
                          conn.prepareStatement(
                              "select count(1) " + queryResult.getExecutedSqlFrom());
                      applyParameters(s, queryResult.getExecutedSqlFromParams());
                    } else {
                      s =
                          conn.prepareStatement(
                              "select count(1) from (" + queryResult.getExecutedSql() + " ) x");
                    ...
                    if (resultRowCount
                        < queryResult.getStartRowNumber() + queryResult.getFetchRowCount()) {
                      queryResult.setFetchRowCount(
                          (int) (resultRowCount - queryResult.getStartRowNumber()));
                    }

                    queryResult
                        .getSqlParams()
                        .add(queryResult.getFetchRowCount()); // queryResult.getStartRowNumber()+
                    if (queryResult.getStartRowNumber() > 0)
                      queryResult.getSqlParams().add(queryResult.getStartRowNumber());

                    sql2.append("select z.*");
                    if (query.getSqlPostSelect() != null
                        && query.getSqlPostSelect().trim().length() > 2) {
                      if (query.getSqlPostSelect().contains("${")) {
                        Object[] oz =
                            DBUtil.filterExt4SQL(
                                query.getSqlPostSelect(),
                                queryResult.getScd(),
                                queryResult.getRequestParams(),
                                null);
                      ...
                      } else sql2.append(", ").append(query.getSqlPostSelect());
                      sql2.append(" ");
                    }
                    if (queryResult.getPostProcessQueryFields() != null
                        && mainTable != null
                    ...
                    for (W5QueryField qf : queryResult.getQuery().get_queryFields())
                      if ((qf.getPostProcessTip() == 12 || qf.getPostProcessTip() == 13)
                          && qf.getLookupQueryId()
                              != 0) { // queryField'da postProcessTip=lookupTable olanlar saptaniyor
                    ...
                    for (W5QueryField qf : queryResult.getQuery().get_queryFields())
                      if ((qf.getPostProcessTip() == 16 || qf.getPostProcessTip() == 17)
                          && qf.getLookupQueryId()
                              != 0) { // queryField'da postProcessTip=lookupQuery olanlar saptaniyor
                        W5QueryResult queryFieldLookupQueryResult =
                        ...
                        if (queryFieldLookupQueryResult != null
                          ...
                          if (qf.getPostProcessTip() == 16
                              && queryFieldLookupQueryResult.getQuery().get_queryFields().size()
                                  > 2)
                            for (int qi = 2;
                                qi
                                    < queryFieldLookupQueryResult
                                        .getQuery()
                                        .get_queryFields()
                                        .size();
                                qi++) {
                              W5QueryField qf2 =
                                  queryFieldLookupQueryResult.getQuery().get_queryFields().get(qi);
                    ...
                    if (queryResult.getPostProcessQueryFields() != null
                      ...
                      for (W5QueryField qf : queryResult.getQuery().get_queryFields())
                        if ((qf.getPostProcessTip() == 12 || qf.getPostProcessTip() == 13)
                            && qf.getLookupQueryId()
                                != 0) { // queryField'da postProcessTip=lookupTable olanlar
                          // saptaniyor
                          prepareLookupTableQuery(queryResult, sql2, paramIndex);
                          break;
                        }
                      ...
                      for (W5QueryField qf : queryResult.getQuery().get_queryFields())
                        if ((qf.getPostProcessTip() == 16 || qf.getPostProcessTip() == 17)
                            && qf.getLookupQueryId()
                                != 0) { // queryField'da postProcessTip=lookupQuery olanlar
                          // saptaniyor
                          W5QueryResult queryFieldLookupQueryResult =
                              getQueryResult(queryResult.getScd(), qf.getLookupQueryId());
                          if (queryFieldLookupQueryResult != null
                  ...
                  List<Object[]> resultData =
                      queryResult.getFetchRowCount() == 0
                  ...
                  if (FrameworkSetting.liveSyncRecord
                    ...
                    if (grdOrFcId != 0) {
                      ...
                      String sessionId =
                          mobile
                              ? (String) queryResult.getScd().get("mobileDeviceId")
                              : (String) queryResult.getScd().get("sessionId");
                      liveSyncKeys =
                          UserUtil.getTableGridFormCellCachedKeys(
                              (String) queryResult.getScd().get("projectId"),
                              mainTable.getTableId(),
                              (Integer) queryResult.getScd().get("userId"),
                              sessionId,
                              mobile ? sessionId : queryResult.getRequestParams().get(".w"),
                              queryResult.getRequestParams().get(".t"),
                              grdOrFcId,
                              queryResult.getRequestParams(),
                              true);
                  ...
                  while (rs
                      .next() /* && (maxFetchedCount==0 || totalFetchedCount<maxFetchedCount )*/) {
                    if (newQueryFields == null) {
                      newQueryFields =
                          new ArrayList(
                              queryResult.getQuery().get_queryFields().size()
                      ...
                      if (queryResult.getQueryColMap() != null) {
                        ...
                        for (W5QueryField qf : queryResult.getQuery().get_queryFields())
                          if (qf.getTabOrder() > 0) {
                            W5TableField tf =
                                queryResult.getMainTable() != null && qf.getMainTableFieldId() > 0
                                    ? queryResult
                                        .getMainTable()
                                        .get_tableFieldMap()
                                    ...
                                    : null;
                            if (tf == null
                                || ((GenericUtil.isEmpty(tf.getRelatedSessionField())
                                        || GenericUtil.uInt(
                                                queryResult
                                                    .getScd()
                                                    .get(tf.getRelatedSessionField()))
                                            != 0)
                                    && (tf.getAccessViewUserFields() != null
                                        || GenericUtil.accessControl(
                                            queryResult.getScd(),
                                            tf.getAccessViewTip(),
                                            tf.getAccessViewRoles(),
                                            tf.getAccessViewUsers())))) { // access control
                              newQueryFields.add(qf);
                              if (maxTabOrder < qf.getTabOrder()) maxTabOrder = qf.getTabOrder();
                            }
                          }
                        ...
                        try {
                          ...
                          if (obj != null) {
                            if (obj instanceof java.sql.Timestamp) {
                              try {
                                obj =
                                    (queryResult.getQuery().getQueryTip() == 2
                                            && field.getFieldTip() == 2)
                    ...
                    if (query.getDataFillDirectionTip() != 0) resultData.add(0, o);
                    else resultData.add(o);
      ...
      throw new IWBException(
          ...
          "Query",
          queryResult.getQueryId(),
          GenericUtil.replaceSql(
              sql2.length() == 0 ? queryResult.getExecutedSql() : sql2.toString(),
              queryResult.getSqlParams()),
          "[8," + queryResult.getQueryId() + "] " + queryResult.getQuery().getDsc(),
          e);
    } finally {
  ...
  public Map<String, Object> runSQLQuery2Map(
      String sql, List params, List<W5QueryField> queryFields) {
    return runSQLQuery2Map(sql, params, queryFields, true);
  }

  public List runQuery2Map(
      Map<String, Object> scd, int queryId, Map<String, String> requestParams) {
    W5QueryResult queryResult = getQueryResult(scd, queryId);
    if (queryId != 1
        && queryId != 824
    ...
    if (!GenericUtil.isEmpty(requestParams.get("sort"))) {
      if (requestParams.get("sort").equals(FieldDefinitions.queryFieldName_Comment)) {
        //	queryResult.setOrderBy("coalesce((select qz.last_comment_id from iwb.w5_comment_summary
        // qz where qz.table_id= AND qz.table_pk= AND qz.customization_id=),0) DESC");
  ...
  public Map<String, Object> runSQLQuery2Map(
    ...
    try {
      return getCurrentSession()
          .doReturningWork(
              new ReturningWork<Map<String, Object>>() {

                public Map<String, Object> execute(Connection conn) throws SQLException {
                  PreparedStatement s =
                      conn.prepareStatement("select x.* from (" + sql + ")  x  limit 1");
                  ...
                  if (rs.next()) {
                    ...
                    if (queryFields == null || queryFields.size() == 0) {
                      ...
                      for (int columnIndex = 1;
                          columnIndex <= rsm.getColumnCount();
                          columnIndex++) {
                        String columnName =
                            rsm.getColumnName(columnIndex).toLowerCase(FrameworkSetting.appLocale);
                        Object obj = rs.getObject(columnIndex);
                        if (obj == null) continue;
                        if (obj instanceof java.sql.Timestamp) {
                          try {
                            result.put(
                                columnName, GenericUtil.uFormatDateTime((java.sql.Timestamp) obj));
                          } catch (Exception e) {
                          }
                        } else if (obj instanceof java.sql.Date) {
                          try {
                            ...
                            try {
                              result.put(
                                  columnName, GenericUtil.uFormatDateSade((java.sql.Date) obj));
                            } catch (Exception e2) {
                            }
                          }
                            ...
                            case 2: // locale
                              result.put(
                                  ...
                                  LocaleMsgCache.get2(
                                      ...
                                      (String) o)); // TODO. burasi scd'deki degerler olacak
                              break;
                            default:
                  ...
                  if (FrameworkSetting.hibernateCloseAfterWork)
                    if (closeConnectionAfterRun && conn != null) conn.close();
                  return result;
                }
              });
      ...
      throw new IWBException(
          "sql", "Custom.Query.Map", 0, GenericUtil.replaceSql(sql, params), "Error Executing", e);
    }
  }
  ...
  private void logQueryAction(Log5QueryAction action, W5QueryResult queryResult, String error) {
    ...
    if (queryResult.getQuery().getLogLevelTip() == 2
        || (queryResult.getQuery().getLogLevelTip() == 1
            && FrameworkSetting.logQueryActionMinTime <= action.getProcessTime())) {
      if (FrameworkSetting.log2tsdb) {
        action.setDsc(
            GenericUtil.replaceSql(queryResult.getExecutedSql(), queryResult.getSqlParams()));
      } else {
        action.setDsc(
            GenericUtil.uStrMax(
                GenericUtil.replaceSql(queryResult.getExecutedSql(), queryResult.getSqlParams()),
  ...
  private void logGlobalFuncAction(
      Log5GlobalFuncAction action, W5GlobalFuncResult fr, String error) {
    if (fr.getGlobalFunc().getLogLevelTip() == 0
        || FrameworkCache.getAppSettingIntValue(fr.getScd(), "log_db_func_action") == 0) return;
    action.calcProcessTime();
    if ((fr.getGlobalFunc().getLogLevelTip() == 1)
        || (fr.getGlobalFunc().getLogLevelTip() == 2
            && FrameworkCache.getAppSettingIntValue(fr.getScd(), "log_db_func_action") != 0
            && FrameworkCache.getAppSettingIntValue(fr.getScd(), "log_db_func_action_mintime")
                <= action.getProcessTime())) {
      action.setDsc(GenericUtil.replaceSql(fr.getExecutedSql(), fr.getSqlParams()));
      saveObject(action);
    }
  }

  public void loadFormCellLookups(
      Map<String, Object> scd,
      List<W5FormCellHelper> formCellResults,
      Map<String, String> requestParams,
      String tabId) {
    ...
    for (W5FormCellHelper rc : formCellResults)
      try {
        ...
        switch (c.getControlTip()) {
          case 100: // button
            if (c.getInitialSourceTip() == 4) { // sql ise
              rc.setExtraValuesMap(
                  runSQLQuery2Map(
                      GenericUtil.filterExt(c.getInitialValue(), scd, requestParams, null)
                          .toString(),
                      null,
                      null));
            }
          ...
          case 9: // remote query
            ...
            // c.set_lookupListCount(c.getLookupQueryId()); // Fake: Normalde Query Id tutulur, ama
          ...
          case 6: // eger static combobox ise listeyi load et
            ...
            List<W5LookUpDetay> oldList =
                !FrameworkCache.hasQueuedReloadCache(projectId, "13." + lookUp.getLookUpId())
          ...
          case 55: // tree combo and treepanel
            String includedParams =
                GenericUtil.filterExt(c.getLookupIncludedParams(), scd, requestParams, null)
                    .toString();
            ...
            if (lookupQueryResult.getQuery().getQuerySourceTip() != 15)
              switch (lookupQueryResult.getQuery().getQuerySourceTip()) {
                case 1376: // WS Method
                  W5WsMethod wsm =
                      FrameworkCache.getWsMethod(
                          projectId, lookupQueryResult.getQuery().getMainTableId());
            ...
            if (c.getControlTip() == 10 || c.getControlTip() == 23 || c.getControlTip() == 7) {
              if (c.getDialogGridId() != 0) {
                ...
                rc.getExtraValuesMap()
                    .put(
                        "dialogGrid", getGridResult(scd, c.getDialogGridId(), requestParams, true));
              }

              if (c.getControlTip() == 10 && GenericUtil.isEmpty(rc.getValue()))
                break; // advanced select ise ve degeri yoksa hicbirsey koyma
            }

            if (lookupQueryResult.getErrorMap().isEmpty()) {
              runQuery(lookupQueryResult);
              if (tabId != null
                  && lookupQueryResult.getQuery().getMainTableId() != 0
              ...
              if (paramMap.get("xapproval_id") != null
                  && c.getLookupQueryId() == 606) { // onaylanmis ve reddedildiyi koy
                W5Workflow ta =
                    FrameworkCache.getWorkflow(
                        projectId, GenericUtil.uInt(paramMap.get("xapproval_id")));
        ...
        throw new IWBException(
            "framework",
            "Load.FormElement",
            rc.getFormCell().getFormCellId(),
            null,
            "[41," + rc.getFormCell().getFormCellId() + "]",
            e);
      }
  }
  ...
  public void loadFormTable(W5FormResult formResult) {
    ...
    if (formResult.getFormCellResults() == null)
      formResult.setFormCellResults(
          new ArrayList<W5FormCellHelper>(formResult.getForm().get_formCells().size()));
    for (W5FormCell cell : formResult.getForm().get_formCells()) {
      if (cell.getObjectDetailId() != 0) {
        ...
        if (tf != null && tf.getTabOrder() > 0) {
          if (tf.getAccessViewTip() != 0 && tf.getAccessViewUserFields() != null) {
            if ((!GenericUtil.accessControl(
                ...
                && (!GenericUtil.isEmpty(tf.getAccessViewUserFields())
                    && accessUserFieldControl(
                        t,
                        tf.getAccessViewUserFields(),
                        formResult.getScd(),
          ...
          if (log5LogId != null) {
            sql.append("t.")
                .append(((W5TableField) cell.get_sourceObjectDetail()).getDsc())
                .append(",");
          } else {
            sql.append("t.")
                .append(((W5TableField) cell.get_sourceObjectDetail()).getDsc())
    ...
    try {
              ...
              new Work() {
                ...
                public void execute(Connection conn) throws SQLException {
                          ...
                          try {
                            if (cellResult.getFormCell().getControlTip() == 18) { // date time
                              obj =
                                  rs.getTimestamp(
                                      ((W5TableField)
                                              cellResult.getFormCell().get_sourceObjectDetail())
                                          .getDsc());
                              obj = GenericUtil.uFormatDateTime((java.sql.Timestamp) obj);
                            } else // date
                      ...
                      switch (cellResult.getFormCell().getInitialSourceTip()) {
                        ...
                        case 1: // request
                          cellResult.setValue(
                              formResult
                                  .getRequestParams()
                                  .get(cellResult.getFormCell().getInitialValue()));
                          break;
                        case 2:
                          Object o =
                              formResult.getScd().get(cellResult.getFormCell().getInitialValue());
                          cellResult.setValue(o == null ? null : o.toString());
                          break;
                        case 3: // app_setting
                          cellResult.setValue(
                              FrameworkCache.getAppSettingStringValue(
                                  formResult.getScd(), cellResult.getFormCell().getInitialValue()));
                          break;
                        case 4: // SQL
                          Object[] oz =
                              DBUtil.filterExt4SQL(
                          ...
                          if (oz[1] != null && ((List) oz[1]).size() > 0) {
                            Map<String, Object> m =
                                runSQLQuery2Map(oz[0].toString(), (List) oz[1], null, false);
                            if (m != null)
                          ...
                          try {
                            if (FrameworkSetting.rhinoInstructionCount > 0)
                              cx.setInstructionObserverThreshold(
                                  FrameworkSetting.rhinoInstructionCount);
                            ...
                            sc.append("\nvar _request=")
                                .append(
                                    GenericUtil.fromMapToJsonString(formResult.getRequestParams()));
                            ...
                            if (res != null && res instanceof org.mozilla.javascript.Undefined)
                              res = null;
                            if (res != null
                                && ((W5Param) cellResult.getFormCell().get_sourceObjectDetail())
                                        .getParamTip()
                                    == 4) res = "" + new BigDecimal(res.toString()).intValue();
                            ...
                            throw new IWBException(
                                "rhino",
                                "FormElement",
                                cellResult.getFormCell().getFormCellId(),
                                sc.toString(),
                                "[41," + cellResult.getFormCell().getFormCellId() + "]",
                                e);
                          } finally {
                  ...
                  throw new IWBException(
                        ...
                        t.getTableId(),
                        GenericUtil.replaceSql(sql.toString(), realParams),
                        "[15," + t.getTableId() + "] Loaded more then 1 record",
                        null);
                  if (rs != null) rs.close();
                  if (s != null) s.close();
                  ...
                  if (pkField2 != null) {
                    ...
                    if (FrameworkCache.getAppSettingIntValue(
                                formResult.getScd(), "file_attachment_flag")
                            != 0
                    ...
                    if (FrameworkCache.getAppSettingIntValue(
                                formResult.getScd(), "make_comment_flag")
                            != 0
                        && t.getMakeCommentFlag() != 0) {
                      if (extraSql.length() > 10) extraSql.append(",");
                      if (FrameworkCache.getAppSettingIntValue(
                              formResult.getScd(), "make_comment_summary_flag")
                          != 0) {
                        extraSql.append(
                    ...
                    */
                    ...
                    if (extraSql.length() > 10) {
                      ...
                      applyParameters(
                          s, params); // romisUtil.replaceSql(extraSql.toString(),params)
                      rs = s.executeQuery();
                      if (rs.next()) {
                        ...
                        if (extrInfoSet.contains("comment_count")) {
                          formResult.setCommentCount(
                              GenericUtil.uInt(rs.getObject("comment_count")));
      ...
      throw new IWBException(
          ...
          formResult.getFormId(),
          null,
          "[40," + formResult.getFormId() + "] Form Load Exception",
          e);
    } finally {
      //    		session.close();
    }
    ...
    W5Page page =
        (W5Page)
            getCustomizedObject(
                ...
                tr.getTemplateId(),
                projectId,
                "Page"); // ozel bir client icin varsa
    tr.setPage(page);
    page.set_pageObjectList(
        find(
            "from W5PageObject t where t.activeFlag=1 AND t.templateId=? AND t.projectUuid=? order by t.tabOrder",
            tr.getTemplateId(),
            projectId));
    ...
    if (GenericUtil.isEmpty(page.getCode()) || page.getCode().startsWith("!"))
      for (W5PageObject to : page.get_pageObjectList())
        if (to.getSrcQueryFieldId() != null && to.getDstQueryParamId() != null) {
          ...
          String sql =
              "select (select f1.dsc from iwb.w5_query_field f1 where f1.project_uuid=? AND f1.query_field_id=?) f_dsc, "
                  + "(select p1.dsc from iwb.w5_query_param p1 where p1.project_uuid=? AND p1.query_param_id=?) p_dsc";
          if (to.getDstStaticQueryParamId() != null
              && !GenericUtil.isEmpty(to.getDstStaticQueryParamVal())) {
            p.add(projectId);
            p.add(to.getDstStaticQueryParamId());
            sql +=
                ",(select p1.dsc from iwb.w5_query_param p1 where p1.project_uuid=? AND p1.query_param_id=?) ps_dsc";
          }
  ...
  public W5CardResult getDataViewResult(
      Map<String, Object> scd,
      int dataViewId,
      Map<String, String> requestParams,
  ...
  public W5GridResult getGridResult(
    ...
    try {
      ...
      if (!gridResult.getFormCellResultMap().isEmpty())
        loadFormCellLookups(
            gridResult.getScd(),
            new ArrayList(gridResult.getFormCellResultMap().values()),
            gridResult.getRequestParams(),
            null);
      ...
      throw new IWBException(
          ...
          gridId,
          null,
          "[5," + gridId + "]" + (g != null ? " " + g.getDsc() : ""),
          e);
    }
  }

  public String getInitialFormCellValue(
      Map<String, Object> scd, W5FormCell cell, Map<String, String> requestParams) {
    String result = null;
    switch (cell.getInitialSourceTip()) {
      ...
      case 10: // approvalStates
        ...
        for (W5WorkflowStep step : app.get_approvalStepList())
          if (GenericUtil.accessControl(
              scd, (short) 1, step.getApprovalRoles(), step.getApprovalUsers()))
  ...
  public Map<String, String> interprateSmsTemplate(
      W5FormSmsMail fsm,
      Map<String, Object> scd,
      Map<String, String> requestParams,
      int fsmTableId,
  ...
  public W5Email interprateMailTemplate(
      W5FormSmsMail fsm,
      Map<String, Object> scd,
      Map<String, String> requestParams,
      int fsmTableId,
      ...
      case 1:
        ...
        if (g == null) {
          g =
              (W5Grid)
                  getCustomizedObject(
                      "from W5Grid g where g.gridId=? AND g.projectUuid=?",
                      form.getObjectId(),
                      projectId,
                      "Grid");
        }
        if (g != null) {
          ...
          if (q == null) {
            q =
                (W5Query)
                    getCustomizedObject(
                        "from W5Query g where g.mainTableId>0 AND g.queryId=? AND g.projectUuid=?",
                        g.getQueryId(),
                        projectId,
                        "Query");
          }
          if (q != null) t = FrameworkCache.getTable(projectId, q.getMainTableId()); // grid
        }
        break;
      ...
      if (!onlyFreeFields || cell.getObjectDetailId() == 0)
        try {
          if (t != null) {
            ...
            if (form.getObjectTip() == 2
                && cell.get_sourceObjectDetail() != null
            ...
            if (tf != null) {
              if (!GenericUtil.accessControl(
                  ...
                  tf.getAccessInsertUsers())) continue; // access control
              if (!GenericUtil.isEmpty(tf.getRelatedSessionField())
                  && GenericUtil.uInt(formResult.getScd().get(tf.getRelatedSessionField())) == 0)
                continue;
            }
          ...
          switch (cell.getInitialSourceTip()) {
            ...
            case 3: // app_setting
              result.setValue(
                  FrameworkCache.getAppSettingStringValue(
                      formResult.getScd(), cell.getInitialValue()));
              break;
            case 4: // SQL
              Object[] oz =
                  DBUtil.filterExt4SQL(
            ...
            case 5: // CustomJS(Rhino)
              ...
              try {
                  ...
                  if (em != null) {
                    formResult
                        ...
                        .put(
                            cell.getDsc(),
                            LocaleMsgCache.get2(
                                0, (String) formResult.getScd().get("locale"), em.toString()));
                    continue;
                  }
                }
                /*				Object exp = scope.get("expression", scope);
                ...
                throw new IWBException(
                    "rhino", "Backend JS Code", 0, sc.toString(), "Error Executing", e);
              } finally {
                // Exit from the context.
                Context.exit();
              }
            ...
            case 10: // approvalStates
              String selectedItems = "";
              W5Workflow app =
                  FrameworkCache.getWorkflow(projectId, GenericUtil.uInt(cell.getInitialValue()));
              if (app != null)
                for (W5WorkflowStep step : app.get_approvalStepList())
                  if (GenericUtil.accessControl(
                      formResult.getScd(),
                      (short) 1,
                      step.getApprovalRoles(),
                      step.getApprovalUsers())) selectedItems += "," + step.getApprovalStepId();
          ...
          throw new IWBException(
              ...
              cell.getFormCellId(),
              null,
              "[41," + cell.getFormCellId() + "] " + cell.getDsc(),
              e);
        }
  }
  ...
  private void loadGrid(W5GridResult gr) {
    ...
    W5Grid grid =
        (W5Grid)
            getCustomizedObject(
                "from W5Grid t where t.gridId=? and t.projectUuid=?",
                gr.getGridId(),
                projectId,
                "Grid"); // ozel bir client icin varsa
    gr.setGrid(grid);

    grid.set_gridColumnList(
        find(
            "from W5GridColumn t where t.projectUuid=? AND t.gridId=? order by t.tabOrder",
            projectId,
            gr.getGridId()));
      ...
      case 1:
        if (grid.getRowColorFxQueryFieldId() != 0) { // lookup eslesme
          grid.set_listCustomGridColumnRenderer(
              find(
                  "from W5CustomGridColumnRenderer t where t.projectUuid=? AND t.gridId=? AND t.queryFieldId=?",
                  projectId,
                  gr.getGridId(),
                  grid.getRowColorFxQueryFieldId()));
        }
        break;
      case 2:
      case 3:
        if (grid.getRowColorFxQueryFieldId() != 0 || grid.getRowColorFxTip() == 3) { // kosul
          grid.set_listCustomGridColumnCondition(
              find(
                  "from W5CustomGridColumnCondition t where t.projectUuid=? AND t.gridId=? AND t.queryFieldId=? order by t.tabOrder",
                  projectId,
                  gr.getGridId(),
                  grid.getRowColorFxQueryFieldId()));
        }
    ...
    if (grid.get_pkQueryField() == null) {
      if (FrameworkSetting.debug)
        throw new IWBException(
            "framework", "Grid", grid.getGridId(), null, "Grid PK Missing", null);
    ...
    for (W5GridColumn column : grid.get_gridColumnList()) {
      ...
      if (column.getFormCellId() > 0) { // form_cell
        W5FormCell cell =
            (W5FormCell)
                getCustomizedObject(
                    "from W5FormCell t where t.formCellId=? and t.projectUuid=?",
                    column.getFormCellId(),
                    projectId,
                    null);
        if (cell != null) {
    ...
    grid.set_menuItemList(
        find(
            "from W5ObjectMenuItem t where t.objectTip=? AND t.objectId=? AND t.projectUuid=? order by t.tabOrder",
            (short) 5,
            gr.getGridId(),
            projectId));
    if (grid.getColumnRenderTip() != 0)
      grid.set_gridModuleList(
          find(
              "from W5GridModule t where t.gridId=? AND t.projectUuid=? order by t.tabOrder",
              gr.getGridId(),
              projectId));
  ...
  public boolean updateFormTable(W5FormResult formResult, String paramSuffix) {
    ...
    if (formResult.getApprovalRecord() != null) {
      approvalStep =
          FrameworkCache.getWorkflow(scd, formResult.getApprovalRecord().getApprovalId())
              .get_approvalStepMap()
    ...
    for (W5FormCell x : f.get_formCells())
      if (x.getNrdTip() != 1
          && x.getObjectDetailId() != 0
          && x.get_sourceObjectDetail() != null
        ...
        if (approvalStep != null
            ...
            && !GenericUtil.hasPartInside(
                approvalStep.getUpdatableFields(), "" + tf.getTableFieldId())) continue;
        if (tf.getAccessViewTip() != 0
            && !GenericUtil.accessControl(
                scd, tf.getAccessViewTip(), tf.getAccessViewRoles(), tf.getAccessViewUsers())
            && (GenericUtil.isEmpty(tf.getAccessViewUserFields())
                || accessUserFieldControl(
                    t,
                    tf.getAccessViewUserFields(),
                    scd,
                    formResult.getRequestParams(),
                    paramSuffix))) continue;
        if (tf.getAccessUpdateTip() != 0
            && !GenericUtil.accessControl(
                scd, tf.getAccessUpdateTip(), tf.getAccessUpdateRoles(), tf.getAccessUpdateUsers())
        ...
        if (moduleMap != null && moduleMap.get(x.getFormModuleId()) != null) {
          ...
          if (!GenericUtil.accessControl(
              scd,
              module.getAccessViewTip(),
              module.getAccessViewRoles(),
              module.getAccessViewUsers())) continue;
        }

        if (paramSuffix.length() > 0
            && formResult.getRequestParams().get(x.getDsc() + paramSuffix) == null) continue;
        if (x.getControlTip() == 31
            && GenericUtil.uInt(x.getLookupIncludedValues()) == 1
        ...
        Object psonuc =
            GenericUtil.prepareParam(
                ...
                null,
                x.getNotNullFlag(),
                x.getDsc() + paramSuffix,
                x.getDefaultValue(),
                formResult.getErrorMap());

        if (formResult.getErrorMap().isEmpty()) {
          if (x.getFormCellId() == 6060
              || x.getFormCellId() == 16327
              || x.getFormCellId() == 16866) { // mail sifre icin
    ...
    if (usedFields.isEmpty()) { // sorun var
      throw new IWBException(
          "validation", "Form Update", formResult.getFormId(), null, "No Used Fields", null);
    }
    if (f.get_versioningFlag()) { // eger versionin varsa, version'lari degistir ve version_no'yu
      // arttir
    ...
    try {
      ...
      throw new IWBException(
          ...
          "Form.Update",
          formResult.getFormId(),
          GenericUtil.replaceSql(sql.toString(), updateParams),
          "Error Updating",
          e);
    } finally {
      //			session.close();
  ...
  public int copyFormTable(
    ...
    for (W5FormCell x : f.get_formCells())
      if (x.getNrdTip() != 1
          && x.getObjectDetailId() != 0
          && x.getControlTip()
              < 100) { // disabled(1) degil VE freeField(getObjectDetailId()!=0) degilse
        ...
        if (
        /*!PromisUtil.accessControl(formResult.getScd(), p1.getAccessViewTip(), p1.getAccessViewRoles(), p1.getAccessViewUsers()) ||*/
        !GenericUtil.accessControl(
            formResult.getScd(),
            p1.getAccessInsertTip(),
            p1.getAccessInsertRoles(),
            p1.getAccessInsertUsers())) continue; // access control
    ...
    if (usedFields.isEmpty()) { // sorun var
      throw new IWBException(
          "validation", "Form Copy", formResult.getFormId(), null, "No Used Fields", null);
    }

    if (f.get_versioningFlag()) { // eger versioning varsa, version'lari degistir ve version_no'yu
      // arttir
    ...
    try {
          ...
          .doReturningWork(
              new ReturningWork<Integer>() {
                ...
                public Integer execute(Connection conn) throws SQLException {
                  ...
                  for (Integer o :
                      calculatedParams
                          .keySet()) { // calculated ve output edilecek parametreler hesaplaniyor
                    // once
      ...
      throw new IWBException(
          ...
          "Form.Copy",
          formResult.getFormId(),
          GenericUtil.replaceSql(sql.toString(), copyParams),
          "Error Copying",
          e);
    } finally {
      //			session.close();
  ...
  public Object[] listObjectCommentAndAttachUsers(
    ...
    boolean recordInsertUserFlag = false,
        recordVersionUserFlag = false,
        assignedUserFlag = false,
    ...
    if (!l.isEmpty()) {
      ...
      if (recordInsertUserFlag || recordVersionUserFlag || assignedUserFlag) {
        ...
        List<Object[]> l2 =
            customizationFlag
                ? executeSQLQuery(sql.toString(), tablePk, customizationId)
        ...
        if (l2 != null && !l2.isEmpty()) {
          ...
          if (recordVersionUserFlag)
            extraUserIds.add(
                GenericUtil.uInt(fieldCount == 1 ? l2.get(0) : l2.get(0)[recordVersionUserId]));
          if (assignedUserFlag)
            extraUserIds.add(
                GenericUtil.uInt(fieldCount == 1 ? l2.get(0) : l2.get(0)[assignedUserId]));
        }
      }
    }
    /* TODO
  ...
  public int insertFormTable(W5FormResult formResult, String paramSuffix) {
    ...
    if (usedFields.isEmpty()) { // sorun var
      throw new IWBException(
          "validation", "Form Insert", formResult.getFormId(), null, "No Used Fields", null);
    }
    ...
    try {
          ...
          .doReturningWork(
              new ReturningWork<Integer>() {
                ...
                public Integer execute(Connection conn) throws SQLException {
                  ...
                  if (t.getTableId() != 329
                      && FrameworkCache.getAppSettingIntValue(customizationId, "make_comment_flag")
                          != 0
                      && t.getMakeCommentFlag() != 0) {
                    PreparedStatement s2 =
                        conn.prepareStatement(
                    ...
                    applyParameters(
                        ...
                        formResult
                            .getOutputFields()
                            .get(t.get_tableParamList().get(0).getExpressionDsc()),
                        projectId,
                        t.getTableId(),
      ...
      throw new IWBException(
          ...
          "Form.Insert",
          formResult.getFormId(),
          GenericUtil.replaceSql(sql.toString(), insertParams),
          "Error Inserting",
          e);
    } finally {
      //			session.close();
  ...
  public boolean deleteTableRecord(W5FormResult formResult, String paramSuffix) {
    ...
    if (FrameworkSetting.vcs && FrameworkSetting.vcsServer && t.getVcsFlag() != 0)
      throw new IWBException(
          ...
          formResult.getFormId(),
          null,
          "VCS Server not allowed to delete VCS Table",
          null);
    StringBuilder sql = new StringBuilder();
  ...
  private void organizeQueryFields4WSMethod(
      Map<String, Object> scd, final W5Query q, final short insertFlag) {
    ...
    final Map<String, W5QueryFieldCreation> existField =
        new HashMap<String, W5QueryFieldCreation>();
    final List<Object> sqlParams = new ArrayList();
    String projectId = (String) scd.get("projectId");
    List<W5QueryFieldCreation> existingQueryFields =
        find(
            "from W5QueryFieldCreation t where t.queryId=? AND t.projectUuid=?",
            q.getQueryId(),
            projectId);
    ...
    if (q.getSqlSelect().equals("*")) {
      ...
      List<W5WsMethodParam> outParams =
          find(
              ...
              q.getMainTableId(),
              parentParam.getWsMethodParamId(),
              projectId);
      int j = 0;
  ...
  private void organizeQueryFields4TSMeasurement(
      Map<String, Object> scd, final W5Query q, final short insertFlag) {
    final Map<String, W5QueryFieldCreation> existField =
        new HashMap<String, W5QueryFieldCreation>();
    final List<Object> sqlParams = new ArrayList();
    String projectId = (String) scd.get("projectId");
    List<W5QueryFieldCreation> existingQueryFields =
        find("from W5QueryFieldCreation t where t.queryId=? AND t.projectUuid=?", q.getQueryId());
    for (W5QueryFieldCreation field : existingQueryFields) {
      existField.put(field.getDsc().toLowerCase(FrameworkSetting.appLocale), field);
    }
  }
  ...
  public void organizeQueryFields(
    ...
    for (final W5Query query :
      ...
      if (query.getSqlGroupby() != null
          && query.getSqlGroupby().trim().length() > 0
          && query.getQueryTip() != 9) // group by connect olmayacak
      sql.append(" group by ").append(query.getSqlGroupby().trim());
      if (query.getSqlPostSelect() != null && query.getSqlPostSelect().trim().length() > 2) {
        sql =
            new StringBuilder(sql.length() + 100)
                .append("select z.*,")
                .append(query.getSqlPostSelect())
                .append(" from (")
                .append(sql)
                .append(") z");
      }
      ...
      if (oz[1] != null) sqlParams.addAll((List) oz[1]);
        ...
        for (int qi = 0; qi < sqlStr.length(); qi++)
          if (sqlStr.charAt(qi) == '?') sqlParams.add(null);
      ...
      try {
                ...
                new Work() {
                  ...
                  public void execute(Connection conn) throws SQLException {
                    ...
                    for (int i = 1, j = 0; i <= columnNumber; i++) {
                      String columnName =
                          meta.getColumnName(i).toLowerCase(FrameworkSetting.appLocale);
                      if (insertFlag != 0
                          && existField.get(columnName)
                              == null) { // eger daha onceden boyle tanimlanmis bir field yoksa
                        W5QueryFieldCreation field = new W5QueryFieldCreation();
                        field.setDsc(columnName);
                        field.setCustomizationId((Integer) scd.get("customizationId"));
                        if (columnName.equals("insert_user_id")
                            || columnName.equals("version_user_id"))
        ...
        for (W5QueryFieldCreation field :
            existField.values()) { // icinde bulunmayanlari negatif olarak koy
        ...
        if (queryId != -1)
          throw new IWBException(
              "sql", "QueryField.Creation", queryId, sql.toString(), "Error Creating", e);
      }
    }
    boolean vcs = FrameworkSetting.vcs;
    if (insertFlag != 0 && insertList != null)
  ...
  public void reloadJobsCache() {
    ...
    try {
      ...
      FrameworkCache.wJobs.addAll(
          find("from W5JobSchedule x where x.activeFlag=1 and x.actionStartTip=1"));
  ...
  public void reloadLocaleMsgsCache2(int cid) {
    ...
    List<Object[]> l =
        (List<Object[]>)
            executeSQLQuery(
                "select x.customization_id, x.locale, x.locale_msg_key, x.dsc from iwb.w5_locale_msg x "
                    + whereSql
  ...
  public void reloadErrorMessagesCache(int customizationId) { // TODO
    ...
    /*		List l = executeSQLQuery("select exc_code, locale_msg_key from iwb.w5_exception_filter order by exc_id");
  ...
  public void reloadApplicationSettingsCache(int cid) {
    if (cid == -1) FrameworkCache.appSettings.clear();
    else FrameworkCache.appSettings.remove(cid);
    List<Object[]> lm =
        (List<Object[]>)
            executeSQLQuery(
                "select customization_id, dsc, val from iwb.w5_app_setting where ?=-1 OR customization_id=?",
                cid,
                cid);
    if (lm != null)
      for (Object[] m : lm) {
    ...
    int default_customization_id =
        Integer.parseInt(FrameworkCache.appSettings.get(0).get("default_customization_id"));
  ...
  public void addProject2Cache(String projectId) {
    ...
    if (l.isEmpty())
      throw new IWBException(
          "framework", "Not Valid Project", 0, projectId, "Not Valid Project", null);
  ...
  public List<W5Project> reloadProjectsCache(int cid) {
    ...
    List<W5Project> lp =
        cid == -1
            ? (List<W5Project>) find("from W5Project t")
...
//    FrameworkSetting.preloadWEngine = FrameworkCache.getAppSettingIntValue(0, "preload_engine");
    ...
    //		FrameworkSetting.profilePicture = FrameworkCache.getAppSettingIntValue(0,
    ...
    //		if(FrameworkSetting.preloadWEngine!=0)FrameworkCache.clearPreloadCache(); //TODO

    FrameworkSetting.advancedSelectShowEmptyText =
        FrameworkCache.getAppSettingIntValue(0, "advanced_select_show_empty_text") != 0;
    FrameworkSetting.simpleSelectShowEmptyText =
        FrameworkCache.getAppSettingIntValue(0, "simple_select_show_empty_text") != 0;
    FrameworkSetting.cacheTimeoutRecord =
        FrameworkCache.getAppSettingIntValue(0, "cache_timeout_record") * 1000;
    FrameworkSetting.crudLogSchema =
        FrameworkCache.getAppSettingStringValue(
            0, "log_crud_schema", FrameworkSetting.crudLogSchema);
    FrameworkSetting.mailSchema =
        FrameworkCache.getAppSettingStringValue(0, "mail_schema", FrameworkSetting.mailSchema);
    ...
    FrameworkSetting.onlineUsersAwayMinute =
        1000 * 60 * FrameworkCache.getAppSettingIntValue(0, "online_users_away_minute", 3);
    FrameworkSetting.onlineUsersLimitMinute =
        1000 * 60 * FrameworkCache.getAppSettingIntValue(0, "online_users_limit_minute", 10);
    FrameworkSetting.onlineUsersLimitMobileMinute =
        1000
            * 60
            * FrameworkCache.getAppSettingIntValue(
                0, "online_users_limit_mobile_minute", 7 * 24 * 60); // 7 gun
    FrameworkSetting.tableChildrenMaxRecordNumber =
        FrameworkCache.getAppSettingIntValue(0, "table_children_max_record_number", 100);

    FrameworkSetting.mailPassEncrypt =
        FrameworkCache.getAppSettingIntValue(0, "encrypt_mail_pass") != 0;

    FrameworkSetting.mobilePush = FrameworkCache.getAppSettingIntValue(0, "mobile_push_flag") != 0;
    FrameworkSetting.mobilePushProduction =
        FrameworkCache.getAppSettingIntValue(0, "mobile_push_production_flag") != 0;

    FrameworkSetting.workflow = FrameworkCache.getAppSettingIntValue(0, "approval_flag") != 0;
    FrameworkSetting.liveSyncRecord =
        FrameworkCache.getAppSettingIntValue(0, "live_sync_record") != 0;

    FrameworkSetting.lookupEditFormFlag =
        FrameworkCache.getAppSettingIntValue(0, "lookup_edit_form_flag") != 0;
    //		PromisSetting.replaceSqlSelectX = PromisCache.getAppSettingIntValue(0,
  ...
  public void reloadLookUpCache(String projectId) {
    ...
    for (W5LookUp lookUp :
        (List<W5LookUp>)
            find("from W5LookUp t where t.projectUuid=? order by  t.lookUpId", projectId)) {
      lookUp.set_detayList(new ArrayList());
      lookUp.set_detayMap(new HashMap());
      subMap.put(lookUp.getLookUpId(), lookUp);
    }

    for (W5LookUpDetay lookUpDetay :
        (List<W5LookUpDetay>)
            find(
                "from W5LookUpDetay t where t.projectUuid=? order by t.lookUpId, t.tabOrder",
                projectId)) {
  ...
  public void reloadMobileCache() { // TODO
    ...
    if (l != null)
      for (Object[] o : l) {
        //			UserUtil.addDevice(o[0].toString(), GenericUtil.uInt(o[1]), GenericUtil.uInt(o[2]),
  ...
  public void reloadConversionsCache(String projectId) {
    ...
    List<W5Conversion> cnvAll =
        (List<W5Conversion>) find("from W5Conversion t where t.projectUuid=?", projectId);
  ...
  public void reloadTablesCache(String projectId) {
    ...
    List<W5Table> lt =
        (List<W5Table>) find("from W5Table t where t.projectUuid=? order by t.tableId", projectId);
  ...
  public void reloadUsersCache(String projectId) { // customizationID ??
    ...
    if (l != null)
      for (Object[] m : l) {
        UserUtil.addUserWithProfilePicutre(
            GenericUtil.uInt(m[1]),
            (String) m[2],
            (String) m[3],
            GenericUtil.uInt(m[4]) != 0,
            GenericUtil.uInt(m[5]));
      }
  }
    ...
    List<Object[]> l =
        (List<Object[]>)
            executeSQLQuery(
                "select x.customization_id, x.user_id, x.user_name, x.dsc, 1 allow_multi_login_flag, x.profile_picture_id from iwb.w5_user x "
                    + (customizationId >= 0
                        ? (" where x.customization_id=" + customizationId + "")
                        : ""));
    if (l != null)
      for (Object[] m : l) {
        UserUtil.addUserWithProfilePicutre(
  ...
  public void reloadTableParamListChildListParentListCache(String projectId) { // TODO
    ...
    List<W5TableParam> tplAll =
        (List<W5TableParam>)
            find(
                "from W5TableParam t where t.projectUuid=? order by t.tableId, t.tabOrder",
                projectId);
    ...
    for (W5TableParam tp : tplAll) {
      if (lastTableId != tp.getTableId()) {
        if (x != null) {
          ...
          if (tx != null) tx.set_tableParamList(x);
    ...
    List<W5TableChild> tcAll =
        (List<W5TableChild>)
            find("from W5TableChild t where t.projectUuid=? order by t.tableId", projectId);
    ...
    for (W5TableChild tc : tcAll) {
      ...
      if (lastTableId != tc.getTableId()) {
        W5Table tx =
            FrameworkCache.getTable(projectId, lastTableId == -1 ? tc.getTableId() : lastTableId);
        if (tx != null) {
  ...
  public void reloadTableFiltersCache(String projectId) { // customizationID ??
    /*	for(W5TableFilter tf:(List<W5TableFilter>)find("from W5TableFilter t where t.projectUuid=?", projectId)){
  ...
  public void reloadFrameworkCaches(int customizationId) {
    logger.info(
        "Caching Started (users, jobs, localeMsgs, errorMsgs, appSettings, lookUps, tableParams)");
    ...
    if (customizationId == -1) {
      customizationList =
          (List<W5Customization>) find("from W5Customization t order by t.customizationId");
      reloadMobileCache();
      setApplicationSettingsValues();
      FrameworkCache.cachedOnlineQueryFields =
          (List<W5QueryField>)
              find(
      ...
      customizationList =
          (List<W5Customization>)
              find("from W5Customization t where t.customizationId=?", customizationId);

    for (W5Customization c : customizationList)
      FrameworkCache.wCustomizationMap.put(c.getCustomizationId(), c);

    reloadUsersCache(customizationId);
    ...
    if (ll != null)
      for (Object[] oo : ll) UserUtil.addProjectUsers(oo[0].toString(), oo[1].toString());
    ll =
        executeSQLQuery(
...
private void reloadWsServersCache(String projectId) {
    ...
    for (W5WsServer o : lt) {
      ...
      o.set_methods(
          (List<W5WsServerMethod>)
              find(
                  "from W5WsServerMethod t where t.projectUuid=? AND t.wsServerId=? order by t.tabOrder",
                  projectId,
                  o.getWsServerId()));
  ...
  public void reloadWsClientsCache(String projectId) {
    ...
    List<W5WsMethod> lm =
        find("from W5WsMethod x where x.projectUuid=? order by x.wsId, x.wsMethodId", projectId);
    for (W5WsMethod m : lm) {
  ...
  private void reloadFeed(int customizationId) {
    ...
    Query q =
        getCurrentSession()
            .createQuery("from Log5Feed t where t.customizationId=? order by t.feedId desc");
    q.setInteger(0, customizationId);
    List<Log5Feed> l =
        q.setFirstResult(0).setMaxResults(FrameworkSetting.feedLoadAtStartupDepth).list();
  ...
  public void executeGlobalFunc(W5GlobalFuncResult r, String paramSuffix) {
    ...
    if (r.getGlobalFunc().getLkpCodeType() == 1) {
      ...
      try {
            ...
            if (p1.getOutFlag() == 0) {
              ...
              Object o =
                  GenericUtil.prepareParam(
                      ...
                      null,
                      (short) 0,
                      p1.getSourceTip() == 1 ? p1.getDsc() + paramSuffix : null,
                      null,
                      r.getErrorMap());
              if (o == null) sc.append("null");
                  ...
                  || (o instanceof Boolean)) sc.append(o);
              else if ((o instanceof Date))
                sc.append("'").append(GenericUtil.uFormatDate((Date) o)).append("'");
              else sc.append("'").append(o).append("'");
        ...
        sc.append("\nvar _scd=")
            ...
            .append(";\nvar _request=")
            .append(GenericUtil.fromMapToJsonString2(r.getRequestParams()))
            .append(";\n")
            .append(script);
        ...
        if(scope.has("errorMsg", scope)){
        	...
        	if(em!=null)throw new PromisException("rhino","GlobalFuncId", r.getGlobalFuncId(), script, LocaleMsgCache.get2(0,(String)r.getScd().get("locale"),em.toString()), null);
        } */
        if (hasOutParam) {
        ...
        throw new IWBException(
            ...
            r.getGlobalFuncId(),
            script,
            "[20," + r.getGlobalFuncId() + "] " + r.getGlobalFunc().getDsc(),
            e);
      } finally {
        // Exit from the context.
        cx.exit();
                ...
                public void execute(Connection conn) throws SQLException {
                  ...
                  for (int ix = 0; ix < sqlParams.size(); ix++) {
                    ...
                    if (sqlNames.get(ix) != null) {
                      ...
                      if (p1.getOutFlag() == 1) s.registerOutParameter(i, t1);
                    } else {
                  ...
                  switch (r.getGlobalFunc().getDbFuncId()) {
                    case 925: // reload locale msg cache
                      LocaleMsgCache.set2(
                          (Integer) r.getScd().get("customizationId"),
    ...
    /*if(PromisCache.getAppSettingIntValue(r.getScd(), "bpm_flag")!=0){
    	int nextBpmActionId = bpmControl(r.getScd(), r.getRequestParams(), r.getGlobalFunc().get_listBpmStartAction(), r.getGlobalFunc().get_listBpmEndAction(), 11, 0, 0);
  ...
  public void bookmarkForm(
      String dsc, int formId, int userId, int customizationId, W5FormResult formResult) {
  ...
  public W5JasperResult getJasperResult(
      Map<String, Object> scd, W5JasperReport jasperreport, Map<String, String> parameterMap) {
    ...
    W5Jasper jasper =
        (W5Jasper)
            find("from W5Jasper t where t.jasperId=?", jasperreport.getJasperId())
                .get(0); // ozel bir client icin varsa
    jasperResult.setJasper(jasper);
    jasper.set_jasperObjects(
        find(
            "from W5JasperObject t where t.jasperId=? order by t.tabOrder",
            jasperreport.getJasperId()));
  ...
  public void copyTableRecord(int tableId, int tablePk, String srcSchema, String dstSchema) {
    ...
    try {
      ...
      if (FrameworkSetting.debug) e.printStackTrace();
      if (dstSchema != null
          && !dstSchema.equals(FrameworkCache.getAppSettingStringValue(0, "default_schema"))) {
        b.setLength(0);
  ...
  public String getTableFields4VCS(W5Table t, String prefix) {
    ...
    if (t == null || GenericUtil.isEmpty(t.get_tableFieldList())) return s.toString();
    for (W5TableField f : t.get_tableFieldList()) {
      if (f.getTabOrder() < 0 || f.getDsc().equals(t.get_tableFieldList().get(0).getDsc()))
        continue;
      if (f.getDsc().equals("version_no")
          || f.getDsc().equals("insert_user_id")
          || f.getDsc().equals("insert_dttm")
          || f.getDsc().equals("version_user_id")
          || f.getDsc().equals("version_dttm")
          || f.getDsc().equals("customization_id")
          || f.getDsc().equals("project_uuid")) continue;
      ...
      switch (f.getParamTip()) {
        ...
        case 2:
          s.append("case when ")
              ...
              .append(".")
              .append(f.getDsc())
              .append(" is null then '' else to_char(")
              .append(prefix)
              .append(".")
              .append(f.getDsc())
  ...
  public boolean copyFormTableDetail(
      W5FormResult masterFormResult,
      W5TableChild tc,
      String newMasterTablePk,
      String schema,
      String prefix) {
    ...
    W5Table mt =
        FrameworkCache.getTable(
            scd, masterFormResult.getForm().getObjectId()); // .get_sourceTable();
    ...
    if (t.getAccessViewTip() == 0 && !FrameworkCache.roleAccessControl(scd, 0)) {
      // throw new PromisException("security","Module", 0, null, "Modul Kontrol: Erişim kontrolünüz
      // yok", null);
      return false;
    }
    if (!GenericUtil.accessControl(
        scd, t.getAccessViewTip(), t.getAccessViewRoles(), t.getAccessViewUsers())) {
      // throw new PromisException("security","Form", t.getDefaultUpdateFormId(), null, "Tablo
    ...
    String detailStaticFieldDsc =
        tc.getRelatedStaticTableFieldId() > 0
    ...
    detailSql +=
        "exists(select 1 from "
            ...
            + masterFieldDsc
            + " = x."
            + detailFieldDsc // detail ile iliski
            + " AND q."
            + mt.get_tableParamList().get(0).getExpressionDsc()
            + " = ?"; // master kayit ile iliski
    ...
    requestParams.put(
        detailFieldDsc,
        GenericUtil.isEmpty(newMasterTablePk)
            ? requestParams.get(mt.get_tableParamList().get(0).getDsc())
            : newMasterTablePk);
    if (rl != null)
      for (Object o : rl) {
        if (multiKey) {
        ...
        } else requestParams.put(t.get_tableParamList().get(0).getDsc(), o.toString());
          ...
          for (W5TableChild tc2 : t.get_tableChildList())
            if (tc2.getCopyStrategyTip() == 1) {
              copyFormTableDetail(
                  formResult,
                  tc2,
                  requestParams.get(t.get_tableFieldList().get(0).getDsc()),
                  schema,
                  prefix);
            }
        }
  ...
  public List<Map> getMainTableData(
      Map<String, Object> scd, int tableId, String tablePk) { // TODO: yetki eksik
  ...
  public List<W5TableRecordHelper> findRecordParentRecords(
      Map<String, Object> scd,
      int tableId,
      int tablePk,
      int maxLevel,
      boolean includeSummarySqlFlag) { // TODO:includeSummarySqlFlag ise yaramiyor
    ...
    while (t != null && tablePk != 0 && level < maxLevel) {
      level++;
      W5TableRecordHelper trh =
          null; // FrameworkCache.getTableCacheValue(t.getCustomizationId(), tableId,tablePk);
      if (trh != null
          && (currentMillis - trh.getLastAccessTime())
              < FrameworkSetting.cacheTimeoutRecord) { // caching den aliyorum
        ...
        sql.append("'WARNING!!! Not defined SummarySql' dsc");
          ...
          if (!t.getSummaryRecordSql().contains(" dsc,")
              && !t.getSummaryRecordSql().endsWith(" dsc")) sql.append(" dsc");
        }
        if (t.get_tableParentList() != null && t.get_tableParentList().size() > 0) {
          sql.append(",x.");
          tc = t.get_tableParentList().get(0);
          sql.append(t.get_tableFieldMap().get(tc.getRelatedTableFieldId()).getDsc())
              .append(" ptable_pk");
        ...
        if (accessControlFlag)
          sql.append(
              ...
              .append(t.getTableId())
              .append(
                  " AND ac.customization_id=${scd.customizationId} AND cast(ac.table_pk as int8)=x.")
              .append(t.get_tableParamList().get(0).getExpressionDsc());
        sql.append(" where x.")
            .append(t.get_tableParamList().get(0).getExpressionDsc())
            .append("=${req.")
        ...
        if (m != null) {
          if (accessControlFlag)
            trh.setViewAccessControl(
                new W5AccessControlHelper(
                    (String) m.get("access_roles"), (String) m.get("access_users")));
          if (m.size() == 1) {
          ...
          //					FrameworkCache.putTableCacheValue(t.getCustomizationId(), tableId, tablePk,
          // trh);//cache le objeyi
          switch (ptCount) {
            case 0:
              return l;
  ...
  public Object interprateTemplateExpression(
      Map<String, Object> scd,
      Map<String, String> requestParams,
      int tableId,
      int tablePk,
    ...
    sql.append("select ");
    List<W5TableFieldCalculated> ltfc =
        find(
    ...
    for (int bas = tmp1.indexOf("${"); bas >= 0; bas = tmp1.indexOf("${", bas + 2)) {
      ...
      if (subStr.startsWith("scd.")) { // session
        ...
        params.add(reqStr);
          ...
          || (subStr.startsWith("lnk.")
              && subStr.substring(4).replace(".", "&").split("&").length
                  == 1)) { // direk tablodan veya link'in ilk kaydi ise
        ...
        if (!fieldFound) invalidKeys.add(subStr);
        ;
      } else if (subStr.startsWith("lnk.")
          && subStr.substring(4).replace(".", "&").split("&").length
              > 1) { // burda bu field ile olan baglantiyi cozmek lazim TODO
        String newSubStr = subStr.substring(4);
        ...
        if (sss.length > 1) { // TODO kaldirilmasi lazim
          ...
          for (int isss = 0; isss < sss.length - 1; isss++) {
            ...
            for (W5TableField tf : newT.get_tableFieldList())
              if (tf.getDsc().equals(sss[isss])) {
                foundSt = false;
                if (tf.getDefaultControlTip() == 7
                    || tf.getDefaultControlTip() == 9
                    || tf.getDefaultControlTip()
                        == 10 /* || tf.getDefaultControlTip()==15*/) { // sub table
                  ...
                  newSub
                      ...
                      .append(isss)
                      .append(".")
                      .append(st.get_tableFieldList().get(0).getDsc())
                      .append("=")
          ...
          if (foundSt && newSub.length() > 0) {
            tmp1.replace(
                bas, bit + 1, "(" + newSub.toString() + ")"); // .put(subStr, o.toString());
          }
        }
      }
    }
  ...
  public Map<String, String> interprateTemplate(
      Map<String, Object> scd,
      Map<String, String> requestParams,
      int tableId,
      int tablePk,
    ...
    // ${lnk.field_name1.field_name2.field_name3} -> link oldugu tablodaki field in linkindeki diger
    // vs.vs.
    // smsMailReplaceTip : 0->yok, 1:sms, 2:mail
    // conversionTip : 0->Serbest, 1: lookup-mapping, 2: sql, 3: javascript, 4: expression, 5:
    // serbest&''
    ...
    List<W5TableFieldCalculated> ltfc =
        find(
            "from W5TableFieldCalculated t where t.projectUuid=? AND t.tableId=? order by t.tabOrder",
            FrameworkCache.getProjectId(scd.get("projectId"), "15." + tableId),
            tableId);
    W5Table t = FrameworkCache.getTable(scd, tableId);
      ...
      if (subStr.startsWith("scd.")) { // session
        ...
        for (W5TableField tf : t.get_tableFieldList())
          if (tf.getDsc().equals(newSubStr)) {
            if (tf.getDefaultControlTip() == 15) {
              ...
              if (st != null
                  && smsMailReplaceTip != 0
                  && smsMailTableIds.contains(
                      st.getTableId())) { // eger sms/mail isi ve bu bir link ise
                res.put(subStr, fieldPrefix + field_cnt);
                resField.put(subStr, tf);
                sql.append("iwb.fnc_sms_mail_adress_multi(?,")
                    .append(st.getTableId())
                    .append(",")
                    .append("x.")
                    .append(newSubStr)
                    .append(",")
                    .append(smsMailReplaceTip)
                    .append(") ")
                    .append(fieldPrefix)
                    .append(field_cnt)
                    .append(",");
              ...
              else sql.append("x.").append(newSubStr);
        ...
        for (W5TableFieldCalculated tfc : ltfc)
          if (tfc.getDsc().equals(newSubStr)) {
            ...
            else sql.append("(").append(sqlCode).append(")");
            sql.append(" ").append(fieldPrefix).append(field_cnt).append(",");
            field_cnt++;
            break;
          }
        ...
        if (sss.length > 1) {
              ...
              if (tf.getDsc().equals(sss[isss])) {
                if (tf.getDefaultControlTip() == 7
                  ...
                  if (isss == sss.length - 2
                      && summaryMust) { // burda t'deki summary record sql varsa
                    if (smsMailReplaceTip != 0) {
                      if (smsMailTableIds.contains(dltId)) // eger sms/mail isi ve bu bir link ise
                      newSub
                            .append("iwb.fnc_sms_mail_adress(?,")
                            .append(dltId)
                            .append(",")
                            .append("y")
                            .append(isss)
                            ...
                            .append(smsMailReplaceTip)
                            .append(")");
                      else break; // HATA: sms mail tipinde olmasi gereken link, degil
                    } else
                      newSub
                          .append("iwb.fnc_lookup_table_summary(?,")
                          .append(dltId)
                          .append(",")
                          .append("y")
                          .append(isss)
          ...
          if (newSub.length() > 0) {
            ...
            field_cnt++;
            for (W5TableField tf : newT.get_tableFieldList())
              if (tf.getDsc().equals(sss[sss.length - 1])) resField.put(subStr, tf);
          }
        } else if (sss.length == 1) { // direk lnk.field_name seklinde olanlar icin
          for (W5TableField tf : t.get_tableFieldList())
            if (tf.getDsc().equals(sss[0])) {
              if ((conversionTip == 0)
                  && (tf.getDefaultControlTip() == 7
                      || tf.getDefaultControlTip() == 9
                      || tf.getDefaultControlTip() == 10)
                  && tf.getDefaultLookupTableId() != 0) { // sub table
                W5Table st = FrameworkCache.getTable(scd, tf.getDefaultLookupTableId());
                if (st == null) break; // HATA: gerekli bir alt kademe tabloya ulasilamadi

                if (smsMailReplaceTip != 0) {
                  if (smsMailTableIds.contains(
                      tf.getDefaultLookupTableId())) // eger sms/mail isi ve bu bir link ise
                  sql.append("iwb.fnc_sms_mail_adress(?,")
                        .append(tf.getDefaultLookupTableId())
                        .append(",")
                        .append("x.")
                        .append(sss[0])
                        .append(",")
                  ...
                  sql.append("iwb.fnc_lookup_table_summary(?,")
                      .append(tf.getDefaultLookupTableId())
                      .append(",")
                      .append("x.")
                      .append(sss[0])
                      .append(")");
                params.add(scd.get("userRoleId"));
              } else sql.append("x.").append(sss[0]);
              sql.append(" ").append(fieldPrefix).append(field_cnt).append(",");
              res.put(subStr, fieldPrefix + field_cnt);
              field_cnt++;
            }
    ...
    } else qres = new HashMap();

    for (String keyz : res.keySet()) {
      if (res.get(keyz) != null) {
        if (keyz.startsWith("req.")
            || keyz.startsWith("scd.")
            || keyz.startsWith("app.")
            || keyz.equals("company_logo")) newRes.put(keyz, res.get(keyz));
    ...
    if (replace)
      ...
      for (String subStr : resField.keySet()) { // replace yok ama herseyin degismesi gerek
        W5TableField tf = resField.get(subStr);
        if (tf != null)
          switch (tf.getDefaultControlTip()) {
            case 6: // combo static
  ...
  public boolean accessUserFieldControl(
      W5Table t,
      String accessUserFields,
      Map<String, Object> scd,
      Map<String, String> requestParams,
      String paramSuffix) {
  ...
  public List getRecordPictures(Map<String, Object> scd, int tableId, String tablePk) {
    ...
    if (l != null)
      for (Object o : l)
        fa.addAll(
            find(
                "from W5FileAttachment t where t.customizationId=? and t.fileAttachmentId=?",
                scd.get("customizationId"),
                GenericUtil.uInt(o)));
    return fa;
  }
  ...
  public Object executeRhinoScript(
      Map<String, Object> scd,
      Map<String, String> requestParams,
      String script,
      Map obj,
    ...
    try {
      ...
      if (requestParams != null)
        sc.append("var _request=")
            .append(GenericUtil.fromMapToJsonString2(requestParams))
            .append(";\n");
      if (script.charAt(0) == '!') script = script.substring(1);
      sc.append(script);
  ...
  public Map interprateConversionTemplate(
      W5Conversion c,
      W5FormResult dstFormResult,
      int conversionTablePk,
      boolean checkCondition,
      boolean onlyForSynch) {
    ...
    if (checkCondition && !GenericUtil.isEmpty(c.getConditionSqlCode())) { // TODO
      boolean b =
          conditionRecordExistsCheck(
              scd,
              dstFormResult.getRequestParams(),
              sf.getObjectId(),
              conversionTablePk,
              c.getConditionSqlCode());
      if (!b) return null;
    }
...
//              cc = GenericUtil.isEmpty(sqlm) ? null : sqlm.get("result").toString();
            ...
            case 6: // Rhino
              Object result =
                  GenericUtil.rhinoValue(
                      executeRhinoScript(
                          dstFormResult.getScd(),
                          dstFormResult.getRequestParams(),
                          cc,
                          null,
                          "result"));
          ...
          for (W5FormCell fc : df.get_formCells())
            if (cCol.getFormCellId() == fc.getFormCellId()) { // buna donusecek
  ...
  public boolean conditionRecordExistsCheck(
      Map<String, Object> scd,
      Map<String, String> requestParams,
      int tableId,
      int tablePk,
      String conditionSqlCode) {
    W5Table t = FrameworkCache.getTable(scd, tableId);
    StringBuilder sql = new StringBuilder();
  ...
  public Map interprateConversionTemplate4WsMethod(
      Map<String, Object> scd,
      Map<String, String> requestParams,
      W5Conversion c,
      int conversionTablePk,
    ...
    for (W5ConversionCol cCol : c.get_conversionColList()) {
      if (!GenericUtil.isEmpty(cCol.getConversionCode())) {
        ...
        switch (cCol.getFieldConversionTip()) {
          ...
          case 6: // Rhino
            Object result =
                GenericUtil.rhinoValue(executeRhinoScript(scd, requestParams, cc, null, "result"));
            if (result != null) cc = result.toString();
            break;
          case 1: // not implemented
          default:
        ...
        for (W5WsMethodParam wsmp : wsm.get_params())
          if (cCol.getFormCellId() == wsmp.getWsMethodParamId()) { // buna donusecek
  ...
  private void addPostQueryFields(
      W5QueryResult queryResult, StringBuilder sql2, AtomicInteger paramIndex) {
    W5Query query = queryResult.getQuery();
    W5Table mainTable = queryResult.getMainTable();
    int customizationId = (Integer) queryResult.getScd().get("customizationId");
    String pkFieldName =
        query.getQueryTip() == 9 ? query.get_queryFields().get(0).getDsc() : "pkpkpk_id";
    if (FrameworkSetting.vcs
        && mainTable.getVcsFlag() != 0 /* && query.getSqlSelect().startsWith("x.*")*/) { // VCS
      //			sql2.append(",(select cx.vcs_object_status_tip from iwb.w5_vcs_object cx where
      // cx.table_id=").append(query.getMainTableId()).append(" AND
      // cx.customization_id=").append(customizationId).append(" AND cx.table_pk=z.pkpkpk_id)
      ...
      sql2.append(",iwb.fnc_vcs_status2(")
          ...
          .append(pkFieldName)
          .append(") ")
          .append(FieldDefinitions.queryFieldName_Vcs)
          .append(" ");
      W5QueryField field = new W5QueryField();
    ...
    if (FrameworkCache.getAppSettingIntValue(queryResult.getScd(), "row_based_security_flag") != 0
      ...
      sql2.append(",(select 1 from iwb.w5_access_control cx where cx.table_id=")
          .append(query.getMainTableId())
          ...
          .append(pkFieldName)
          .append(" limit 1) ")
          .append(FieldDefinitions.queryFieldName_RowBasedSecurity)
          .append(" ");
      W5QueryField field = new W5QueryField();
    ...
    if (FrameworkCache.getAppSettingIntValue(queryResult.getScd(), "file_attachment_flag") != 0
        && mainTable.getFileAttachmentFlag() != 0
        && FrameworkCache.roleAccessControl(queryResult.getScd(), 101)
        && (queryResult.getQueryColMap() == null
            || queryResult
                .getQueryColMap()
                .containsKey(FieldDefinitions.queryFieldName_FileAttachment))) { // fileAttachment
      //			sql2.append(",(select 1 from iwb.w5_file_attachment cx where rownum=1 AND
      // cx.table_id=").append(query.getMainTableId()).append(" AND
      ...
      sql2.append(",(select count(1) from iwb.w5_file_attachment cx where cx.table_id=")
          ...
          .append(" AND cx.table_pk=(z.")
          .append(pkFieldName)
          .append(")::text limit 10) ")
          .append(FieldDefinitions.queryFieldName_FileAttachment)
          .append(" ");
      W5QueryField field = new W5QueryField();
    ...
    if (FrameworkCache.getAppSettingIntValue(queryResult.getScd(), "make_comment_flag") != 0
        && mainTable.getMakeCommentFlag() != 0
        && (queryResult.getQueryColMap() == null
            || queryResult
                .getQueryColMap()
                .containsKey(FieldDefinitions.queryFieldName_Comment))) { // comment
      ...
      if (FrameworkCache.getAppSettingIntValue(queryResult.getScd(), "make_comment_summary_flag")
          != 0) {
        sql2.append(
        ...
        field.setPostProcessTip(
            (short) 48); // extra code : commentCount-commentUserId-lastCommentDttm-viewUserIds-msg
      } else {
    ...
    if (FrameworkCache.getAppSettingIntValue(queryResult.getScd(), "approval_flag") != 0
        && mainTable.get_approvalMap() != null
        && !mainTable.get_approvalMap().isEmpty()
        && (queryResult.getQueryColMap() == null
            || queryResult
                .getQueryColMap()
                .containsKey(FieldDefinitions.queryFieldName_Approval))) { // approval Record
      sql2.append(
          ...
          .append(pkFieldName)
          .append(" limit 1) ")
          .append(FieldDefinitions.queryFieldName_Approval)
          .append(" ");
      W5QueryField field = new W5QueryField();
      ...
      if (FrameworkCache.getAppSettingIntValue(queryResult.getScd(), "toplu_onay") != 0) {
        sql2.append(",(select cx.version_no from iwb.w5_approval_record cx where cx.table_id=")
            ...
            .append(pkFieldName)
            .append(" limit 1) ")
            .append(FieldDefinitions.queryFieldName_ArVersionNo)
            .append(" ");
        field = new W5QueryField();
  ...
  public void saveObject2(Object o, Map<String, Object> scd) {
    ...
    if (o instanceof Log5Notification) {
      ...
      if (n.getTableId() != 0 && n.getTablePk() != 0) {
        ...
        n.set_tableRecordList(
            findRecordParentRecords(scd, n.getTableId(), n.getTablePk(), 0, true));
      }
  ...
  public M5ListResult getMListResult(
    ...
    if (d != null) {
      ...
      d =
          (M5List)
              getCustomizedObject(
                  "from M5List t where t.listId=? and t.projectUuid=?",
                  listId,
                  projectId,
      ...
      W5Query q =
          (W5Query)
              getCustomizedObject(
                  ...
                  d.getQueryId(),
                  projectId,
                  "QueryID"); // ozel bir client icin varsa
      d.set_query(q);
      if (q.getMainTableId() != 0)
        d.set_mainTable(FrameworkCache.getTable(projectId, q.getMainTableId()));
      /*if(d.getDefaultCrudFormId()!=0){
      	W5Form f = (W5Form)getCustomizedObject("from W5Form t where t.formId=? and t.projectUuid=?", d.getDefaultCrudFormId(), projectUuid); // ozel bir client icin varsa
      	f.set_formCells(find("from W5FormCell t where t.formId=? and t.projectUuid=? order by t.tabOrder", d.getDefaultCrudFormId(), projectUuid));
      	d.set_crudForm(f);
      }*/
      Integer searchFormId =
          (Integer)
      ...
      d.set_detailMLists(
          find(
              "from M5List l where l.projectUuid=? AND l.parentListId=? order by l.listId",
              projectId,
              listId));
      mlistResult.setList(d);
      ...
      d.set_toolbarItemList(
          find(
              "from W5ObjectToolbarItem t where t.objectTip=1345 AND t.objectId=? AND t.projectUuid=? order by t.tabOrder",
              listId,
              projectId));

      d.set_menuItemList(
          find(
              "from W5ObjectMenuItem t where t.objectTip=1345 AND t.objectId=? AND t.projectUuid=? order by t.tabOrder",
              listId,
              projectId));
    ...
    if (!noSearchForm && d.get_searchFormId() != 0) {
      ...
      loadFormCellLookups(
          mlistResult.getScd(),
          searchForm.getFormCellResults(),
          mlistResult.getRequestParams(),
          null);
  ...
  public W5ListViewResult getListViewResult(
      Map<String, Object> scd,
      int listViewId,
      Map<String, String> requestParams,
      boolean noSearchForm) {
    ...
    if (!noSearchForm && d.get_searchFormId() != 0) {
      ...
      loadFormCellLookups(
          ...
          searchForm.getFormCellResults(),
          listViewResult.getRequestParams(),
          null);
  ...
  private void loadListView(W5ListViewResult lr) {
    ...
    W5List d =
        (W5List)
            getCustomizedObject(
                "from W5List t where t.listId=? and t.projectUuid=?",
                lr.getListId(),
                projectId,
    ...
    d.set_listColumnList(
        find(
            "from W5ListColumn t where t.projectUuid=? AND t.listId=? order by t.tabOrder",
            projectId,
            lr.getListId()));
    ...
    List<Integer> formIdz =
        find(
            "select t.formId from W5Form t where t.projectUuid=? AND t.objectTip=7 and t.objectId=?",
            projectId,
            lr.getListId());
  ...
  public List<W5TableChildHelper> findRecordChildRecords(
    ...
    for (W5TableChild tc : t.get_tableChildList()) {
      ...
      switch (ct.getAccessViewTip()) {
        case 0:
          ...
          break;
        case 1:
          if (ct.getAccessViewUserFields() == null
              && !GenericUtil.accessControl(
                  scd, ct.getAccessViewTip(), ct.getAccessViewRoles(), ct.getAccessViewUsers())) {
            continue;
          }
      }
      StringBuilder sql = new StringBuilder();
      ...
      if (ct.getMakeCommentFlag() != 0) {
        sql.append(",sum((select count(1) from iwb.w5_comment c where c.project_uuid='")
            .append(scd.get("projectId"))
            .append("' AND c.table_id=")
            .append(ct.getTableId())
            .append(" AND c.table_pk=x.")
            .append(ct.get_tableParamList().get(0).getExpressionDsc())
      ...
      if (ct.getFileAttachmentFlag() != 0) {
        sql.append(",sum((select count(1) from iwb.w5_file_attachment c where c.customization_id=")
            .append(scd.get("customizationId"))
            .append(" AND c.table_id=")
            .append(ct.getTableId())
            .append(" AND c.table_pk=x.")
            .append(ct.get_tableParamList().get(0).getExpressionDsc())
      ...
      sql.append(" from ")
          .append(ct.getDsc())
          .append(" x where x.")
          .append(ct.get_tableFieldMap().get(tc.getRelatedTableFieldId()).getDsc())
          .append("=")
          .append(tablePk);
      if (tc.getRelatedStaticTableFieldId() != 0) {
        sql.append(" AND x.")
            .append(ct.get_tableFieldMap().get(tc.getRelatedStaticTableFieldId()).getDsc())
            .append("=")
      ...
      */
      ...
      if (!GenericUtil.isEmpty(l)) {
        Map m = l.get(0);
        r.add(
            new W5TableChildHelper(
                tc,
                GenericUtil.uInt(m, "xcount"),
                GenericUtil.uInt(m, "xcomment_count"),
  ...
  public boolean accessControlTable(Map<String, Object> scd, W5Table t, Integer tablePk) {
    ...
    if (tablePk != null
        && (!GenericUtil.isEmpty(t.get_approvalMap())
            || (t.getAccessViewTip() != 0
                && ((t.getAccessTips() != null && GenericUtil.hasPartInside2(t.getAccessTips(), "0")
                    || !GenericUtil.isEmpty(
                        t.getAccessViewUserFields())))))) { // TODO : ekstra record bazli kontrol
    }
    return true;
  }
  ...
  public W5TableRecordInfoResult getTableRecordInfo(
    ...
    if ((fieldMap1.get("INSERT_USER_ID") != null || fieldMap1.get("insert_user_id") != null)
      ...
      List params = new ArrayList();
      sql.append(
              "select x.version_no, x.insert_user_id, x.insert_dttm, x.version_user_id, x.version_dttm from ")
          .append(t.getDsc())
          .append(" x where x.")
          .append(t.get_tableParamList().get(0).getExpressionDsc())
          .append("=?");
    ...
    */
    ...
    if (extraSql.length() > 0) {
      ...
      List<Map> l =
          executeSQLQuery2Map("select " + extraSql.append(" ").toString(), params); // from dual
      if (!GenericUtil.isEmpty(l)) {
        Map m = l.get(0);
  ...
  public String getSummaryText4Record(Map<String, Object> scd, int tableId, int tablePk) {
    ...
    if (!GenericUtil.isEmpty(summarySql)) {
      ...
      if (m != null) summaryText = (String) m.get("dsc");
      if (summaryText == null) {
        summaryText = "ERROR: " + sql;
      }
    } else
      summaryText =
          "TODO: make Summary SQL on Table: "
  ...
  public boolean checkIfRecordsExists(Map scd, Map<String, String> requestParams, W5Table t) {
    ...
    for (W5TableParam x : t.get_tableParamList()) {
      ...
      Object psonuc =
          GenericUtil.prepareParam(
              (W5Param) x, scd, requestParams, (short) -1, null, (short) 1, null, null, m);
      params.add(psonuc);
    }
  ...
  public void removeTableChildRecords(
      Map<String, Object> scd, int tableId, int tablePk, String dstDetailTableIds) {
    ...
    for (String dts : dtsl) {
      ...
      for (W5TableChild tc : t.get_tableChildList())
        if (tc.getRelatedTableId() == detailTableId) {
          ...
          params.add(scd.get("customizationId"));
          sql.append(" AND x.")
              .append(dt.get_tableFieldMap().get(tc.getRelatedTableFieldId()).getDsc())
              .append("=?");
          params.add(tablePk);
          if (tc.getRelatedStaticTableFieldId() != 0) {
            sql.append(" AND x.")
  ...
  public W5FileAttachment getFileAttachment(int fileAttachmentId) {
    return (W5FileAttachment)
        find("from W5FileAttachment t where t.fileAttachmentId=?", fileAttachmentId).get(0);
  }

  public String getObjectVcsHash(Map<String, Object> scd, int tableId, int tablePk) {
    W5Table t = FrameworkCache.getTable(scd, tableId);
    StringBuilder s = new StringBuilder();
    s.append("select iwb.md5hash(")
        .append(getTableFields4VCS(t, "x"))
        .append(") xhash from ")
        .append(t.getDsc())
        .append(" x where x.")
        .append(t.get_tableParamList().get(0).getExpressionDsc())
    ...
    if (GenericUtil.isEmpty(l)) return "!";
    else return (String) ((Map) l.get(0)).get("xhash");
  }

  public Map getTableRecordJson(
      Map<String, Object> scd, int tableId, int tablePk, int recursiveLevel) {
    W5Table t = FrameworkCache.getTable(scd, tableId);
    StringBuilder s = new StringBuilder();
    s.append("select x.* from ")
        .append(t.getDsc())
        .append(" x where x.")
        .append(t.get_tableParamList().get(0).getExpressionDsc())
        .append("=?");
  ...
  public boolean saveVcsObject(
      Map<String, Object> scd, int tableId, int tablePk, int action, JSONObject o) { // TODO
    ...
    try {
      ...
      switch (action) {
        case 1: // update
          ...
          for (W5TableField f : t.get_tableFieldList())
            if (f.getTabOrder() > 1) {
              if (f.getDsc().equals("insert_user_id")
                  || f.getDsc().equals("insert_dttm")
                  || f.getDsc().equals("customization_id")
                  || f.getDsc().equals("project_uuid")) continue;
              ...
              try {
                if (o.has(f.getDsc())) {
                  p.add(
                      GenericUtil.getObjectByControl((String) o.get(f.getDsc()), f.getParamTip()));
                } else p.add(null);
              } catch (JSONException e) {
                throw new IWBException(
                    "vcs",
                    "JSONException : saveVcsObject",
                    t.getTableId(),
                    f.getDsc(),
                    e.getMessage(),
                    e);
              ...
              if (GenericUtil.hasPartInside2("insert_dttm,version_dttm", f.getDsc())) {
                ...
                if (f.getDsc().equals("project_uuid")) p.add(scd.get("projectId"));
                  ...
                  try {
                    if (o.has(f.getDsc())) {
                      p.add(
                          GenericUtil.getObjectByControl(
                              (String) o.get(f.getDsc()), f.getParamTip()));
                    } else p.add(null);
                  } catch (JSONException e) {
                    throw new IWBException(
                        "vcs",
      ...
      throw new IWBException(
          "framework", "Save.VCSObject", tablePk, null, "[" + tableId + "," + tablePk + "] ", e);
    }
  ...
  public boolean organizeTable(Map<String, Object> scd, String fullTableName) {
    ...
    if (cnt == 0)
      throw new IWBException(
          "framework", "No Such Table to Define", 0, tableName, "No Such Table to Define", null);
    ...
    if (tableId == 0) {
      ...
      int tableParamId =
          GenericUtil.getGlobalNextval("iwb.seq_table_param", projectUuid, userId, customizationId);
      rq =
          executeUpdateSQLQuery(
      ...
      if (cnt > 0) {
        tableParamId =
            GenericUtil.getGlobalNextval(
                "iwb.seq_table_param", projectUuid, userId, customizationId);
        rq =
            executeUpdateSQLQuery(
                "insert into iwb.w5_table_param "
                    + "(table_param_id, table_id, dsc, expression_dsc, tab_order, param_tip, operator_tip, not_null_flag, source_tip, insert_user_id, version_user_id, project_uuid, customization_id, oproject_uuid)values"
    ...
    l =
        executeSQLQuery2Map(
            "select x.*"
                ...
                + " from information_schema.columns x where x.table_name = ? and x.table_schema = ?",
            p);
    for (Map m : (List<Map>) l) {
      ...
      if (tfId == 0) {
        ...
        int tableFieldId =
            GenericUtil.getGlobalNextval(
                "iwb.seq_table_field", projectUuid, userId, customizationId);
        boolean sessField =
            fieldName.equals("customization_id")
                || fieldName.equals("project_uuid")
        ...
        int rq =
            executeUpdateSQLQuery(
                ...
                xlen == 0
                    ? 0
                    : (xlen == -1 ? GenericUtil.uInt(m.get("character_maximum_length")) : xlen),
                tabOrder,
                userId,
                userId,
                customizationId,
                ...
                GenericUtil.hasPartInside2("insert_user_id,version_user_id", fieldName) ? 336 : 0,
                projectUuid);
  ...
  public void makeDirtyVcsObject(Map<String, Object> scd, int tableId, int tablePk) {
    if (FrameworkSetting.vcsServer)
      throw new IWBException(
          ...
          tableId,
          null,
          "VCS Server not allowed to make Dirt VCS Object",
          null);
    List l =
        find(
    ...
    } else saveObject(new W5VcsObject(scd, tableId, tablePk));
  }
  ...
  public boolean organizeGlobalFunc(Map<String, Object> scd, String fullGlobalFuncName) {
    if (FrameworkSetting.vcsServer)
      throw new IWBException(
          ...
          0,
          fullGlobalFuncName,
          "VCS Server not allowed to organizeGlobalFunc",
          null);
    ...
    if (dbFuncId == 0) {
      dbFuncId =
          GenericUtil.getGlobalNextval("iwb.seq_db_func", projectUuid, userId, customizationId);
      int rq =
          executeUpdateSQLQuery(
    ...
    for (int qi = 0; qi < arp.length; qi++) {
      ...
      if (dbFuncParamId == 0) { // boyle bir kayit yok
        dbFuncParamId =
            GenericUtil.getGlobalNextval(
                "iwb.seq_db_func_param", projectUuid, userId, customizationId);
        executeUpdateSQLQuery(
            ...
            dbFuncParamId,
            dbFuncId,
            arp[qi].equals("puser_role_id")
                ? "userRoleId"
                : (arp[qi].equals("plocale")
                    ? "locale"
                    : (arp[qi].equals("ptrigger_action")
                        ? "triggerAction"
                        : (dbFuncName.startsWith("pcrud_") && qi < 3
                            ? "t" + arp[qi].substring(1)
                            : arp[qi]))),
            arp[qi],
            qi + 1,
            userId,
            userId,
            arp[qi].equals("puser_role_id") || arp[qi].equals("locale") || arp[qi].equals("plocale")
                ? 2
                : 1,
            arp[qi].equals("puser_role_id")
                ? "userRoleId"
  ...
  public String getCurrentDate(int customizationId) {
    return (String)
        executeSQLQuery(
                "select to_char(iwb.fnc_sysdate(?),'" + GenericUtil.dateFormat + "')",
                customizationId)
            .get(0);
  }
  ...
  public Map executeSQLQuery2Map4Debug(
      final Map<String, Object> scd,
      final W5Table t,
      final String sql,
      final List params,
      final int limit,
    ...
    try {
                          ...
                          if (params != null && params.size() > 0)
                            for (int i = 0; i < params.size(); i++) {
                              if (params.get(i) == null) s.setObject(i + 1, null);
                                ...
                                s.setDate(
                                    i + 1, new java.sql.Date(((Date) params.get(i)).getTime()));
                              ...
                              startOffset > 1
                                  ? conn.prepareStatement(
                                      "select q.* from  ("
                                          ...
                                          + startOffset)
                                  : conn.prepareStatement(
                                      "select * from (" + sql + ") q limit " + (limit + 1));
                        } else s = conn.prepareStatement(sql);
                              ...
                              if (f.getDsc().equals(columnName)) {
                                if (f.getDefaultLookupTableId() != 0)
                                  switch (f.getDefaultControlTip()) {
                                    ...
                                    case 15:
                                      W5Table tt =
                                          FrameworkCache.getTable(scd, f.getDefaultLookupTableId());
                                      if (tt != null
                                          && !GenericUtil.isEmpty(tt.getSummaryRecordSql())) {
                                        Map mz = new HashMap();
      ...
      throw new IWBException(
          "sql", "Debug.Query", -1, GenericUtil.replaceSql(sql, params), "Error Executing", e);
    }
  }

  public W5GlobalFuncResult executeGlobalFunc4Debug(
      Map<String, Object> scd, int dbFuncId, Map<String, String> parameterMap) {
    W5GlobalFuncResult r =
        dbFuncId == -1 ? new W5GlobalFuncResult(-1) : getGlobalFuncResult(scd, dbFuncId);
    r.setScd(scd);
    ...
    try {
      ...
      if (dbFuncId != -1 && r.getGlobalFunc().get_dbFuncParamList().size() > 0) {
        ...
        for (W5GlobalFuncParam p1 : r.getGlobalFunc().get_dbFuncParamList())
          if (p1.getOutFlag() == 0) {
            ...
            Object o =
                GenericUtil.isEmpty(s) ? null : GenericUtil.getObjectByTip(s, p1.getParamTip());
            if (o == null) {
              if (p1.getNotNullFlag() != 0)
                r.getErrorMap()
                    .put(p1.getDsc(), LocaleMsgCache.get2(scd, "validation_error_not_null"));
              sc.append("null");
            } else if ((o instanceof Integer)
                || (o instanceof Double)
                || (o instanceof BigDecimal)
      ...
      if (FrameworkSetting.debug)
        se.console(
            "start: " + (r.getGlobalFunc() != null ? r.getGlobalFunc().getDsc() : "new"),
            "DEBUG",
            "info");
      long startTm = System.currentTimeMillis();
      cx.evaluateString(scope, script, null, 1, null);
      r.setProcessTime((int) (System.currentTimeMillis() - startTm));
      //			if(FrameworkSetting.debug)se.console("end: " + (r.getGlobalFunc()!=null ?
      ...
      throw new IWBException(
          ...
          r.getGlobalFuncId(),
          script,
          LocaleMsgCache.get2(0, (String) r.getScd().get("locale"), e.getMessage()),
          e);
    } finally {
      // Exit from the context.
      cx.exit();
  ...
  public void executeQueryAsRhino(W5QueryResult qr, String code) {
    ...
    try {
      ...
      if (q.get_queryParams().size() > 0) {
        ...
        for (W5QueryParam p1 : q.get_queryParams()) {
          ...
          if (o == null) {
            if (p1.getNotNullFlag() != 0)
              qr.getErrorMap()
                  .put(p1.getDsc(), LocaleMsgCache.get2(qr.getScd(), "validation_error_not_null"));
            sc.append("null");
          } else if ((o instanceof Integer)
              || (o instanceof Double)
              || (o instanceof BigDecimal)
      ...
      sc.append("\nvar _scd=")
          .append(GenericUtil.fromMapToJsonString2(qr.getScd()))
          .append(";\nvar _request=")
          .append(GenericUtil.fromMapToJsonString2(qr.getRequestParams()))
          .append(";\n")
          .append(script);
          ...
          if (r instanceof NativeArray) {
            ...
            for (W5QueryField qf : q.get_queryFields())
              if ((qf.getPostProcessTip() == 16 || qf.getPostProcessTip() == 17)
                  && qf.getLookupQueryId() != 0) {
                W5QueryResult queryFieldLookupQueryResult =
                    getQueryResult(qr.getScd(), qf.getLookupQueryId());
                if (queryFieldLookupQueryResult != null
                    && queryFieldLookupQueryResult.getQuery() != null) {
                  W5QueryField field = new W5QueryField();
                  field.setDsc(qf.getDsc() + "_qw_");
                  ...
                  if (qf.getPostProcessTip() == 16
                    ...
                    for (int qi = 2;
                        qi < queryFieldLookupQueryResult.getQuery().get_queryFields().size();
                        qi++) {
                      W5QueryField qf2 =
                          queryFieldLookupQueryResult.getQuery().get_queryFields().get(qi);
                      field = new W5QueryField();
                    ...
                    switch (qf.getFieldTip()) {
                      ...
                      case 8: // json
                        if (o2 instanceof NativeArray) {
                          ...
                          o[qf.getTabOrder() - 1] =
                              GenericUtil.fromNativeArrayToJsonString2Recursive(no2);
                        } else if (o2 instanceof NativeObject) {
                          NativeObject no2 = (NativeObject) o2;
                          o[qf.getTabOrder() - 1] =
                              GenericUtil.fromNativeObjectToJsonString2Recursive(no2);
                        } else if (o2 instanceof Map)
                          o[qf.getTabOrder() - 1] =
                              GenericUtil.fromMapToJsonString2Recursive((Map) o2);
                        else if (o2 instanceof List)
                          o[qf.getTabOrder() - 1] =
                              GenericUtil.fromListToJsonString2Recursive((List) o2);
                        else
                          o[qf.getTabOrder() - 1] =
      ...
      throw new IWBException(
          "rhino", "Query", q.getQueryId(), script, "[8," + q.getQueryId() + "]", e);
    } finally {
      // Exit from the context.
      cx.exit();
    }
    ...
    try {
      ...
      if (!qr.getErrorMap().isEmpty()) {
        throw new IWBException(
            ...
            q.getQueryId(),
            script,
            "Validation ERROR: " + GenericUtil.fromMapToJsonString2(qr.getErrorMap()),
            null);
      }
      sc.append("\nvar _scd=")
          .append(GenericUtil.fromMapToJsonString2(qr.getScd()))
      ...
      if (data == null)
        throw new IWBException(
            "rhino", "QueryId", q.getQueryId(), script, "[result] object not found", null);
      m.put("data", data);
      List fields = new ArrayList();
      for (W5QueryField qf : q.get_queryFields()) {
        Map d = new HashMap();
      ...
      throw new IWBException(
          ...
          q.getQueryId(),
          script,
          LocaleMsgCache.get2(0, (String) qr.getScd().get("locale"), e.getMessage()),
          e);
    } finally {
      // Exit from the context.
      cx.exit();
  ...
  public Map executeQuery4StatWS(W5QueryResult queryResult) {
    ...
    W5WsMethod wsm =
        FrameworkCache.getWsMethod(queryResult.getScd(), queryResult.getQuery().getMainTableId());
    ...
    if (statType > 0 && GenericUtil.isEmpty(funcFields))
      throw new IWBException(
          ...
          queryResult.getQueryId(),
          null,
          LocaleMsgCache.get2(0, (String) scd.get("locale"), "fw_grid_stat_func_fields"),
          null);
  ...
  public Map executeQuery4Stat(
    ...
    int queryId =
        gridId > 0
            ? GenericUtil.uInt(
                executeSQLQuery(
                        "select query_id from iwb.w5_grid g where g.grid_id=? AND g.customization_id=?",
                        gridId,
                        customizationId)
                    .get(0))
            : -gridId;
    ...
    if (qf == null)
      throw new IWBException(
          ...
          queryId,
          null,
          LocaleMsgCache.get2(0, (String) scd.get("locale"), "fw_grid_stat_error"),
          null);
    if (stackFieldId > 0) {
      for (W5QueryField o : query.get_queryFields())
      ...
      if (sf == null)
        throw new IWBException(
            ...
            queryId,
            null,
            LocaleMsgCache.get2(0, (String) scd.get("locale"), "fw_grid_stat_error2"),
            null);
    }
    ...
    if (qf.getFieldTip() == 2) { // date ise
      queryFieldSQL =
          "to_char("
              + queryFieldSQL
              + ", '"
              + (new String[] {"yyyy", "yyyy/Q", "yyyy/mm", "yyyy/WW", "yyyy/mm/dd"}
                  [GenericUtil.uInt(requestParams, "_dtt")])
              + "')";
    }
    ...
    switch (queryResult.getQuery().getQueryTip()) {
      ...
      case 15: // TODO: aslinda hata. olmamasi lazim
        throw new IWBException(
            ...
            queryId,
            null,
            LocaleMsgCache.get2(0, (String) scd.get("locale"), "fw_grid_stat_query_error"),
            null);
      default:
        queryResult.prepareQuery(null);
        if (!queryResult.getErrorMap().isEmpty())
          throw new IWBException(
              ...
              null,
              LocaleMsgCache.get2(
                  0, (String) scd.get("locale"), "fw_grid_stat_query_prepare_error"),
              null);
        ...
        if (statType != 0) {
          ...
          for (W5QueryField o : queryResult.getQuery().get_queryFields())
            if (fqs.contains(o.getQueryFieldId())) {
              count++;
              if (count > 1) sql += "," + stats[statType] + "(" + o.getDsc() + ") xres" + count;
              else sql += stats[statType] + "(" + o.getDsc() + ") xres";
            }
          if (count == 0)
            throw new IWBException(
                "framework",
                ...
                null,
                LocaleMsgCache.get2(
                    0, (String) scd.get("locale"), "fw_grid_stat_match_func_fields_error"),
                null);
        } else {
          sql += "count(1) xres";
        }
              ...
              for (Object k : stackSet)
                if (ld3.get_detayMap().get(k) != null)
                  lm.put(
                      k,
                      GenericUtil.uStrMax(
                          LocaleMsgCache.get2(scd, ld3.get_detayMap().get(k).getDsc()), 20));
                else
                  throw new IWBException(
                      "framework",
                      "QueryField",
                      ...
                      null,
                      LocaleMsgCache.get2(
                          0, (String) scd.get("locale"), "fw_grid_stat_stacked_error"),
                      null);
            break;
          case 12: // table
            for (Object k : stackSet) {
          ...
          switch (o.getPostProcessTip()) {
            ...
            case 12: // lookup table
              for (Map m : l) {
                ...
                if (o2 != null && GenericUtil.uInt(o2) != -999999) {
                  String s =
                      getTableRecordSummary(scd, o.getLookupQueryId(), GenericUtil.uInt(o2), 20);
  ...
  public Map executeQuery4StatTree(
    ...
    int queryId =
        GenericUtil.uInt(
            executeSQLQuery(
                    "select query_id from iwb.w5_grid g where g.grid_id=? AND g.project_uuid=?",
                    gridId,
                    projectId)
                .get(0));
    W5QueryResult queryResult = getQueryResult(scd, queryId);
    ...
    if (tableFieldChain.startsWith("tbl.")
        || (tableFieldChain.startsWith("lnk.")
            && tableFieldChain.substring(4).replace(".", "&").split("&").length
                == 1)) { // direk tablodan veya link'in ilk kaydi ise
      ...
      String newSubStr = tableFieldChain.substring(4);
      boolean fieldFound = false;
      List<W5TableFieldCalculated> ltcf =
          find(
    ...
    if (tableField.getParamTip() == 2
        || tableField.getDefaultControlTip() == 2
        ...
        if (statType != 0) {
          ...
          for (String s : fq) {
            ...
            if (isx < 0) {
              ...
              if (count > 1) sql += "," + stats[statType] + "((" + oo[0] + ")) xres" + count;
              else sql += stats[statType] + "((" + oo[0] + ")) xres";
      ...
      if (l != null) {
        if (!GenericUtil.isEmpty(stackField)) {
          ...
          switch (stackedQueryField.getPostProcessTip()) {
            case 10: // static;
              W5LookUp ld3 =
                  FrameworkCache.getLookUp(projectId, stackedQueryField.getLookupQueryId());
              if (ld3 != null)
                for (Object k : stackSet)
                  if (ld3.get_detayMap().get(k) != null)
                    lm.put(
                    ...
                    throw new IWBException(
                        "framework",
                        "QueryField",
                        stackedQueryField.getQueryFieldId(),
                        null,
                        LocaleMsgCache.get2(
                            0, (String) scd.get("locale"), "fw_grid_stat_stacked_error"),
                        null);
            ...
            case 12: // table
              for (Object k : stackSet) {
                String s =
                    getTableRecordSummary(
                        scd, stackedQueryField.getLookupQueryId(), GenericUtil.uInt(k), 20);
                lm.put(k, GenericUtil.isEmpty(s) ? "Not found for: " + k : s);
              }
              break;
            case 20:
        ...
        switch (tableField.getDefaultControlTip()) {
          case 6: // lookup static
            W5LookUp ld2 =
                FrameworkCache.getLookUp(projectId, tableField.getDefaultLookupTableId());
            if (ld2 != null)
              for (Map m : l) {
                Object o2 = m.get("id");
                if (o2 != null && GenericUtil.uInt(o2) != -999999) {
          ...
          case 10: // lookup table
            for (Map m : l) {
              ...
              if (o2 != null && GenericUtil.uInt(o2) != -999999) {
                String s =
                    getTableRecordSummary(
                        scd, tableField.getDefaultLookupTableId(), GenericUtil.uInt(o2), 20);
                m.put("dsc", GenericUtil.isEmpty(s) ? "Not found for: " + o2 : s);
              }
            }
            break;
  ...
  public List executeQuery4DataList(
    ...
    for (String c : cols) {
      ...
      if (c.startsWith("clc.")) {
        ...
        //				sql.append("").append(l.get(0).getSqlCode().replaceAll("x.", "x.")).append(" as
        ...
        for (int isss = 1; isss < sss.length && isss < 2; isss++) { // TODO simdilik 1 seviye
          if (sss[isss].equals("cld")) {
            ...
            if (!l.isEmpty()) {
              newSub
                  .append("SELECT ")
                  .append("sum" /*valMap.get(c)*/)
                  .append("((")
                  .append(l.get(0).getSqlCode().replaceAll("x.", "z" + isss + "."))
                  .append(")) from ")
                  .append(detT.getDsc())
                  .append(" z")
                  .append(isss)
              ...
              newSub
                  .append(" x.")
                  .append(t.get_tableFieldMap().get(detTc.getTableFieldId()).getDsc())
                  .append("=z")
                  .append(isss)
                  .append(".")
              ...
              if (detTc.getRelatedStaticTableFieldId() > 0
                ...
                newSub
                    ...
                    .append("=")
                    .append(detTc.getRelatedStaticTableFieldVal());
              //							if(detT.get_tableParamList().size()>1)newSub.append(" AND
              ...
              if (tf.getDsc().equals(sss[isss])) {
                ...
                if (detTc.getRelatedStaticTableFieldId() > 0
                  ...
                  newSub
                      ...
                      .append(".")
                      .append(
                          detT.get_tableFieldMap()
                              .get(detTc.getRelatedStaticTableFieldId())
                              .getDsc())
                      .append("=")
                      .append(detTc.getRelatedStaticTableFieldVal());
                //						if(detT.get_tableParamList().size()>1)newSub.append(" AND
                  ...
                  switch (tableField.getDefaultControlTip()) {
                    ...
                    case 9: // lookup
                      if (tableField.getDefaultLookupTableId() > 0) {
                        W5Table dt =
                            FrameworkCache.getTable(scd, tableField.getDefaultLookupTableId());
                        if (dt != null && !GenericUtil.isEmpty(dt.getSummaryRecordSql())) {
                          newSub
                              ...
                              .append(" FROM ")
                              .append(dt.getDsc())
                              .append(" qxq WHERE qxq.")
                              .append(dt.get_tableFieldList().get(0).getDsc())
                              .append("=y")
                              .append(isss)
                              .append(".")
                              .append(sss[isss + 1]);
                          //										if(dt.get_tableParamList().size()>1)newSub.append(" AND
        ...
        if (foundSt && newSub.length() > 0) {
          ...
          throw new IWBException(
              ...
              0,
              null,
              LocaleMsgCache.get2(0, (String) scd.get("locale"), "fw_pivot_field_error"),
              null);
            ...
            if ((f.getDefaultControlTip() == 7 || f.getDefaultControlTip() == 10)
              ...
              if (dt != null && !GenericUtil.isEmpty(dt.getSummaryRecordSql())) {
                sql.append("(SELECT ")
                    ...
                    .append("=x.")
                    .append(f.getDsc());
                //							if(dt.get_tableParamList().size()>1)sql.append(" AND
                // qxq.customization_id=${scd.customizationId}");
              ...
              if (f.getFieldTip() == 2) { // date ise
                sql.append("to_char(x.")
                    .append(f.getDsc())
                    .append(",'")
                    .append(dateFormat)
                    .append("')");
              } else sql.append("x.").append(f.getDsc());
            }
            fieldName = f.getDsc();
            break;
          }
    ...
    if (!GenericUtil.isEmpty(vals)) {
      if (vals.equals("count") || vals.equals(":count")) {
        ...
        for (String s : vals.split(",")) {
          ...
          String fnc =
              (o2.length > 1 && GenericUtil.hasPartInside("sum,count,min,max", o2[1].toLowerCase()))
                  ? o2[1].toLowerCase()
                  : "count";
          if (fnc.equals("count")) {
            //				valMap.clear();break;
      ...
      for (Object[] t : tableNames)
        if (GenericUtil.uInt(
          ...
          if (!GenericUtil.isEmpty(t[1]) && t[1].toString().toLowerCase().startsWith("nextval(")) {
            String seq =
                t[1].toString().substring("nextval(".length() + 1, t[1].toString().length() - 2);
            if (GenericUtil.uInt(
                    executeSQLQuery(
                            ...
                            po.getRdbmsSchema())
                        .get(0))
                > 0) sql.append("drop sequence ").append(seq).append(";");
          }
          sql.append("\n");
        }
      if (sql.length() != 0) {
    ...
    if (ll3 != null)
      for (Object o : ll3) {
        ...
        sql.append("delete from ")
            .append(t.getDsc())
            .append(" x where x.project_uuid=? AND x.oproject_uuid=?");
  ...
  public boolean copyProject(Map scd, String dstProjectId, int dstCustomizationId) { // from, to
    ...
    String schema =
        "c"
            + GenericUtil.lPad(dstCustomizationId + "", 5, '0')
            + "_"
            + dstProjectId.replace('-', '_');
    executeUpdateSQLQuery("set search_path=" + schema);

    npo = (W5Project) find("from W5Project w where w.projectUuid=?", dstProjectId).get(0);

    List<String> doneCommits =
        find(
    ...
    List<W5VcsCommit> sqlCommits =
        find(
            "from W5VcsCommit t where t.commitTip=2 AND t.projectUuid=? order by abs(t.vcsCommitId)",
            srcProjectId);
    ...
    if (ll3 != null)
      for (Object o : ll3) {
        ...
        sql.append("delete from ")
            ...
            .append(t.getDsc())
            .append(" z where z.project_uuid=? AND z.")
            .append(pkField)
            .append("=x.")
            .append(pkField);
        for (W5TableField tf : t.get_tableFieldList())
        ...
        sql.append(" select ")
            ...
            .append(" from ")
            .append(t.getDsc())
            .append(" x where x.project_uuid=? AND not exists(select 1 from ")
            .append(t.getDsc())
            .append(" z where z.project_uuid=? AND z.")
            .append(pkField)
            .append("=x.")
  ...
  public void deleteProjectMetadata(String delProjectId) {
    String[] tables =
        new String[] {
          "w5_table_access_control",
          "w5_ts_measurement_tag",
          "w5_ts_portlet_object",
          "w5_ts_portlet",
          "w5_ts_measurement_field",
          "w5_ts_measurement",
          "w5_ws_look_up_detay",
          "w5_ws_look_up",
          "m5_list_template",
          "w5_ws_server_token",
          "w5_ws_server_method_param",
          "w5_ws_server_method",
          "w5_ws_server",
          "w5_ws_method_param",
          "w5_ws_method",
          ...
          "w5_tutorial",
          "w5_tutorial_user",
          "w5_table_access_condition_sql",
          "w5_table_trigger",
          "w5_form_hint",
          "w5_form_sms_mail_alarm",
          "w5_table_field_calculated",
          "w5_doc",
          "w5_list_column",
          "w5_list",
          "w5_data_view",
          "w5_converted_object",
          "w5_form_sms_mail",
          "w5_custom_grid_column_condtion",
          "w5_custom_grid_column_renderer",
          "w5_table_filter",
          "w5_conversion_col",
          "w5_conversion",
          "w5_feed",
          "w5_uploaded_data",
          "w5_uploaded_import_col_map",
          "w5_uploaded_import_col",
          "w5_uploaded_import",
          "w5_table_child",
          "w5_form_cell_code_detail",
          "w5_help",
          "w5_bi_graph_dashboard",
          "w5_mobile_device",
          "w5_approval_record",
          "w5_approval_step",
          "w5_approval",
