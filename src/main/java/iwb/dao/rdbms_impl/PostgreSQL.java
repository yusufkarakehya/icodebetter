...
public class PostgreSQL extends BaseDAO {
	...
	/*
	 * 
	 * public Object getCustomizedObject(String hql, int objectId, int
	 * customizationId, String onErrorMsg){ List list = find(hql,
	 * objectId,customizationId); if(list.size()==0){ if(onErrorMsg==null)return
						...
						for (W5QueryField qf : queryResult.getQuery().get_queryFields())
							if ((qf.getPostProcessTip() == 16 || qf.getPostProcessTip() == 17)
									&& qf.getLookupQueryId() != 0) { // queryField'da
																		...
																		// olanlar
																		// saptaniyor
								W5QueryResult queryFieldLookupQueryResult = getQueryResult(queryResult.getScd(),
										qf.getLookupQueryId());
					...
					if (c.getControlTip() == 10 || c.getControlTip() == 23 || c.getControlTip() == 7) {
						if (c.getDialogGridId() != 0) {
							...
							rc.getExtraValuesMap().put("dialogGrid",
									getGridResult(scd, c.getDialogGridId(), requestParams, true));
						}
	...
	public W5CardResult getDataViewResult(Map<String, Object> scd, int dataViewId, Map<String, String> requestParams,
		...
		if (!noSearchForm && cr.getCard().get_searchFormId() != 0) {
			...
			loadFormCellLookups(cr.getScd(), searchForm.getFormCellResults(), cr.getRequestParams(), null);
	...
	public W5GridResult getGridResult(Map<String, Object> scd, int gridId, Map<String, String> requestParams,
		...
		try {
			...
			if (!gridResult.getFormCellResultMap().isEmpty())
				loadFormCellLookups(gridResult.getScd(), new ArrayList(gridResult.getFormCellResultMap().values()),
	...
	private void reloadFeed(int customizationId) {
		...
		for (int i = l.size() - 1; i >= 0; i--) {
			...
			if (feed.getTableId() != 0 && feed.getTablePk() != 0) { // detail
																	// icinse
		...
		if (r.getGlobalFunc().getLkpCodeType() == 1) {
			...
			try {
				...
				if (script.contains("$.") || script.contains("$.")) {
					RhinoEngine se = new RhinoEngine(r.getScd(), r.getRequestParams(), this, engine);
					Object wrappedOut = Context.javaToJS(se, scope);
					ScriptableObject.putProperty(scope, "$", wrappedOut);
				}
	...
	public Object executeRhinoScript(Map<String, Object> scd, Map<String, String> requestParams, String script, Map obj,
		...
		try {
			...
			// Collect the arguments into a single string.
			RhinoEngine se = new RhinoEngine(scd, requestParams, this, engine);
			Object wrappedOut = Context.javaToJS(se, scope);
			ScriptableObject.putProperty(scope, "$", wrappedOut);
	...
	public M5ListResult getMListResult(Map<String, Object> scd, int listId, Map<String, String> requestParams,
		...
		if (!noSearchForm && d.get_searchFormId() != 0) {
			...
			initializeForm(searchForm, false);
			loadFormCellLookups(mlistResult.getScd(), searchForm.getFormCellResults(), mlistResult.getRequestParams(),
					null);
	...
	public W5ListViewResult getListViewResult(Map<String, Object> scd, int listViewId,
		...
		if (!noSearchForm && d.get_searchFormId() != 0) {
			...
			initializeForm(searchForm, false);
			loadFormCellLookups(listViewResult.getScd(), searchForm.getFormCellResults(),
	...
	public void executeQueryAsRhino(W5QueryResult qr, String code) {
		...
		try {
			...
			if (script.charAt(0) == '!')
				script = script.substring(1);
			// Collect the arguments into a single string.
			RhinoEngine se = new RhinoEngine(qr.getScd(), qr.getRequestParams(), this, engine);
			Object wrappedOut = Context.javaToJS(se, scope);
			ScriptableObject.putProperty(scope, "$", wrappedOut);
					...
					if (r instanceof NativeArray) {
						...
						for (W5QueryField qf : q.get_queryFields())
							if ((qf.getPostProcessTip() == 16 || qf.getPostProcessTip() == 17)
								...
								W5QueryResult queryFieldLookupQueryResult = getQueryResult(qr.getScd(),
										qf.getLookupQueryId());
								if (queryFieldLookupQueryResult != null
										&& queryFieldLookupQueryResult.getQuery() != null) {
									W5QueryField field = new W5QueryField();
