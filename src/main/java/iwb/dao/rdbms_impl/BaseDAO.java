...
public abstract class BaseDAO {
    ...
    protected void applyParameters(PreparedStatement s, Object... sqlParams) 
            throws SQLException {
    ...
    protected void applyParameters(PreparedStatement s, List<Object> sqlParams)
            throws SQLException {
                ...
                if (params != null && params.length > 0) {
                    for (int i = 0; i < params.length; i++) {
                        if (params[i] == null) {
                            ...
                            s.setDate(i + 1, new java.sql.Date(
    ...
    public List executeSQLQuery2Map(final String sql, final List params) {
        try {
            return getCurrentSession().doReturningWork(
                ...
                while (rs.next()) {
                    ...
                    for (int columnIndex = 1; 
                            columnIndex <= rsm.getColumnCount(); columnIndex++) {
                        ...
                        if (obj instanceof java.sql.Timestamp) {
                            try {
                                result.put(columnName, 
                                        GenericUtil.uFormatDateTimeSade(
            ...
            throw new IWBException("sql", "Custom.Query.ListOfMap", 0, GenericUtil.replaceSql(sql, params), "Error Executing", e);
        }
    }
                    ...
                    if (params.length == 1 && params[0] instanceof List) {
                        ...
                        for (int i = 0; i < params.length; i++) {
                            if (params[i] == null) {
                                ...
                                s.setDouble(i + 1, 
                                        Double.valueOf(params[i].toString()));
                            } else if (params[i] instanceof Integer) {
                                s.setInt(i + 1, 
                                        Integer.valueOf(params[i].toString()));
                            } else {
                                s.setObject(i + 1, params[i]);
                            }
                        }
