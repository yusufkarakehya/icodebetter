...
public class ScriptEngine {
  ...
  */
  ...
  public int globalNextval(String seq) {
    return GenericUtil.getGlobalNextval(
        seq,
  ...
  public void console(Object oMsg, String title, String level) {
      ...
      if (oMsg instanceof String) s = (String) oMsg;
        ...
        if (oMsg != null) {
          if (oMsg instanceof String
              || oMsg instanceof Integer
              || oMsg instanceof Long
              || oMsg instanceof Float
              || oMsg instanceof Double
              || oMsg instanceof BigDecimal) {
            s = oMsg.toString();
          } else if (oMsg instanceof Date || oMsg instanceof Timestamp) {
            s =
                oMsg instanceof Timestamp
                    ? GenericUtil.uFormatDateTime((Timestamp) oMsg)
    ...
    if (scd != null
        ...
        && scd.containsKey("sessionId")
        && requestParams != null
        && requestParams.containsKey(".w"))
      try {
        Map m = new HashMap();
        m.put("success", true);
        m.put("console", s);
  ...
  public Object execFunc(
      int dbFuncId, NativeObject jsRequestParams, boolean throwOnError, String throwMessage) {
    W5GlobalFuncResult result =
        engine.executeFunc(scd, dbFuncId, fromNativeObject2Map(jsRequestParams), (short) 5);
    if (throwOnError && !result.getErrorMap().isEmpty()) {
      throw new IWBException(
          ...
          null,
          throwMessage != null
              ? LocaleMsgCache.get2(scd, throwMessage)
  ...
  public W5FormResult postForm(
      int formId, int action, NativeObject jsRequestParams, String prefix) {
    return postForm(formId, action, jsRequestParams, prefix, true, null);
  }

  public W5FormResult postForm(
      int formId, int action, NativeObject jsRequestParams, String prefix, boolean throwOnError) {
  ...
  public W5FormResult postForm(
      int formId,
      int action,
      NativeObject jsRequestParams,
      String prefix,
      boolean throwOnError,
      String throwMessage) {

    W5FormResult result =
        engine.postForm4Table(scd, formId, action, fromNativeObject2Map(jsRequestParams), prefix);
    if (throwOnError && !result.getErrorMap().isEmpty()) {
      throw new IWBException(
          "rhino",
          "FormId",
  ...
  public Map getTableJSON(
      int tableId, String tablePk, int forAction, boolean throwOnError, String throwMessage) {
    ...
    if (forAction != -1) { // -1:kontrol yok, 0: view, 1: edit, 3:delete
      ...
      if (forAction == 1
          ...
          && !GenericUtil.accessControl(
              scd, t.getAccessUpdateTip(), t.getAccessUpdateRoles(), t.getAccessUpdateUsers())) {
        throw new IWBException(
            "security",
            "Table",
            tableId,
    ...
    if (GenericUtil.isEmpty(l)) {
      if (throwOnError)
        throw new IWBException(
            ...
            tableId,
            null,
            throwMessage != null ? LocaleMsgCache.get2(scd, throwMessage) : "record_not_found",
            null);
      return null;
    }
    Map mo = (Map) l.get(0);
      ...
      if (m != null) {
        ...
        if (m.containsKey("faultcode") && m.containsKey("faultstring")) {
          if (throwFlag)
            throw new IWBException(
                "ws",
                m.get("faultcode").toString(),
                0,
                serviceName,
                m.get("faultstring").toString(),
                null);
          else {
