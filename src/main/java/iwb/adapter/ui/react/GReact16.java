...
public class GReact16 implements ViewAdapter {
...
//		buf.append("}");
	...
	private StringBuilder serializeFormCell(int customizationId,
		...
		case	1:buf.append("type:");
			...
			break;//string
		case	42:buf.append("type:'password'");break;
...
//								break;
	...
	public StringBuilder serializeTemplate(W5PageResult pr) {
		...
		if (page.getPageType() == 11) { // google App Maker page
			code = "return app.showPage(\""+page.getDsc()+"\");";
			
		} else  if (page.getPageType() != 0) { // not html 
...
//			buf3.append("var _CompanyLogoFileId=1;\n");
			...
			if(page.getCode().contains("${components}")) {
				...
				for(W5Page p:pm.values())if(p.getPageType()==11 && !GenericUtil.isEmpty(p.getCode())) {
					bufc.append("\napp.addPage('").append(p.getDsc()).append("',{key:\"").append(p.getTag()).append("\", pageId:").append(p.getPageId()).append(",\njson:").append(p.getCode()).append("});");
					if(!GenericUtil.isEmpty(p.getCssCode())) {
						bufc.append("\napp.addScripts('").append(p.getDsc()).append("',{key:\"").append(p.getTag()).append("\", pageId:").append(p.getPageId()).append(",\nscripts:()=>{\n").append(p.getCssCode()).append("\n}});");
					}
				}
				

				FrameworkCache.addPageResource(pr.getScd(), "js-"+page.getPageId(), bufc.toString());
				code = code.replace("${components}", "<script type=\"text/javascript\" src=\"dyn-res/js-"+page.getPageId()+".js?.x="+page.getVersionNo()+"\" ></script>");
			}
