	...
	private StringBuilder serializeFormCell(int customizationId,
		...
		case	102://label
			if(fc.getLookupQueryId()>=10){
				buf.append("<li class=\"iwb-form-tab iwb-type-").append(fc.getLookupQueryId()).append("\"><div class=\"block-title\" style=\"height: 30px;text-align:center;\">").append(GenericUtil.uStrNvl(value, fieldLabel)).append("</div></li>");
			} else {
			 buf.append("<li><div class=\"block-title iwb-label-").append(fc.getLookupQueryId()).append("\"><i class=\"icon material-icons\" style=\"margin-top: -4px;\">").append(labelMap[fc.getLookupQueryId()]).append("</i>&nbsp; ").append(GenericUtil.uStrNvl(value, fieldLabel)).append("</div></li>");
			}
			break;
		...
		case	5://checkbox
			if(fc.getLookupQueryId()==0){
				buf.append("<li id=\"id-formcell-").append(fc.getFormCellId()).append("\"><div class=\"item-content\"><div class=\"item-inner\"><div class=\"item-input\"><label class=\"toggle toggle-init\"><input type=\"checkbox\"").append(GenericUtil.uInt(value)!=0 ?" checked":"").append(" name=\"").append(fc.getDsc())
				.append("\"");
				buf.append("/><i class=\"toggle-icon\"></i><div class=\"item-title\" style=\"margin-top:-18px; margin-left:50px; color: rgba(0, 0, 0, 0.7);overflow: inherit;font-size: 15px;\">")
