...
public class F7iOS implements ViewMobileAdapter {
...
//								break;
							 ...
							 * appStepId = GenericUtil.uInt(obj);
							 * buf.append("\",\""
							 * ).append(f.getDsc()).append("_qw_\":\""
							 * ).append(FrameworkCache
	...
	public	StringBuilder	serializeList(M5ListResult	listResult){
		...
		buf.append("{success:true, listId:").append(l.getListId()).append(", listTip:").append(l.getListTip()).append(",\n name:'")
			.append(LocaleMsgCache.get2(listResult.getScd(), l.getLocaleMsgKey()))
			.append("',\n htmlData:'").append(GenericUtil.stringToJS2(l.getHtmlDataCode()))
		...
		if(!GenericUtil.isEmpty(l.get_detailMLists())){
			buf.append("\n, recordButtons:[");
			for(M5List d:l.get_detailMLists())buf.append("{icon:'list', text:'").append(LocaleMsgCache.get2(listResult.getScd(), d.getLocaleMsgKey()))
		...
		if(GenericUtil.isEmpty(l.getHtmlPageCode())){
			...
			boolean searchBar = l.getDefaultPageRecordNumber()==0 && l.getListTip()==1;
			...
			if(s3.length()>0)
				s2.append("<div class=\"speed-dial\"><a href=# class=\"floating-button\"><i class=\"icon f7-icons\">add</i><i class=\"icon f7-icons\">close</i></a><div class=\"speed-dial-buttons\">").append(s3).append("</div></div>");  
			...
			s2.append("<div class=\"list-block").append(searchBar ? " search-here searchbar-found":"").append("\"><ul id=\"idx-").append(l.getListId()).append("\">${iwb-data}</ul></div></div></div>");
			buf.append(GenericUtil.stringToJS2(s2.toString()));
		} else //GenericUtil.filterExt(f.getJsCode().substring(0, f.getJsCode().indexOf("${iwb-data}")),listResult.getScd(), listResult.getRequestParams(),null))
	...
	public	StringBuilder serializeGetForm(W5FormResult formResult){
		...
		if (f.getObjectTip() == 2){
			...
			pictureFlag = true;/*FrameworkCache.getAppSettingIntValue(scd, "attach_picture_flag") != 0
					&& t.getFileAttachmentFlag() != 0
					&& t.getAttachPictureFlag() != 0
					&& FrameworkCache.roleAccessControl(scd, t.getModuleId(), 101);*/
...
//		buf.append(GenericUtil.filterExt(fc.getExtraDefinition(), formResult.getScd(), formResult.getRequestParams(), o));
		...
		if(f.getRenderTip()==5 && !GenericUtil.isEmpty(f.getJsCode())){
			...
			if(pictureFlag){
				s.append("<a href=# id=\"idx-photo-").append(formResult.getFormId()).append("\" class=\"link icon-only\"><i class=\"icon icon-camera\"><span id=\"idx-photo-badge-").append(formResult.getFormId()).append("\" class=\"badge bg-red\"");
...
//		    <div class="content-block-title">With Floating Labels</div>
		...
		for (W5FormModule m : ml){
			...
			if(m.getFormModuleId()!=0){
				...
				s.append(GenericUtil.stringToJS(serializeFormCell(customizationId, xlocale,new W5FormCellHelper(fcx), formResult)));
			}
			for (W5FormCellHelper fc : r){
				...
				case	2://date
					jsCode.append("myApp.calendar({input: '#idx-formcell-").append(fc.getFormCell().getFormCellId()).append("',dateFormat: 'dd/mm/yyyy'});\n");
					break;
				case	10://autocomplete
				case	61://autocomplete-multi
					jsCode.append("myApp.autocomplete(iwb.apply({multiple:").append(fc.getFormCell().getControlTip()==61).append(", opener: $$('#idx-formcell-").append(fc.getFormCell().getFormCellId()).append("'), params:'_qid=").append(fc.getFormCell().getLookupQueryId());
...
//				s2.append(",\n\"smsMailTemplateCnt\":").append(cnt).append(",\n\"smsMailTemplates\":[");
				...
				for (W5FormSmsMail fsm : f.get_formSmsMailList())
					if (((fsm.getSmsMailTip() == 0 && FrameworkSetting.sms && FrameworkCache.getAppSettingIntValue(customizationId, "sms_flag") != 0) || 
						...
						s2.append(GenericUtil.stringToJS(serializeFormCell(customizationId, xlocale, fcr, formResult)));
						...
						s2.append("{\"xid\":")
								...
								.append(LocaleMsgCache.get2(customizationId, xlocale, fsm.getDsc()))
								.append(fsm.getPreviewFlag() != 0 ? " (" + (LocaleMsgCache.get2(customizationId, xlocale, "with_preview")) + ")" : "")
								.append("\",\"checked\":")
...
//		for(W5FormCe){}
	...
	private StringBuilder serializeFormCell(int customizationId,
		...
		// int customizationId =
		// GenericUtil.uInt(formResult.getScd().get("customizationId"));
		StringBuilder buf = new StringBuilder();
		...
		case	58: case	59://superbox lov-static, superbox lov-query combo
			boolean multi = fc.getControlTip()!=6 && fc.getControlTip()!=7;
		...
		case	5://checkbox
			...
			.append(fieldLabel).append("</div><div class=\"item-input\"><label class=\"label-switch\"><input type=\"checkbox\"").append(GenericUtil.uInt(value)!=0 ?" checked":"").append(" name=\"").append(fc.getDsc())
...
{"xtype":"combo","xorder_id":4,"xname":"jjjjjjjj","name":"jjjjjjjj","fieldLabel":"jjjjjjjj","labelSeparator":"","listeners":{},"width":200,"value":"","store":[[1,"Value 1"],[2,"Value 2"],[3,"etc..."]]}]}:	
		...
		if(formBuilder.getJSONArray("items")!=null){
			...
			for(int qi=0;qi<items.length();qi++){
				...
				if(c.getControlTip()==2){
					...
					jsCode.append("myApp.calendar({input: '#idx-formcell-").append(formCellId).append("',dateFormat: 'dd/mm/yyyy'});\n");
	...
	public StringBuilder serializeGraphDashboard(W5BIGraphDashboard gd, Map<String, Object> scd){
		...
		if(gd.getLegendFlag()!=0)buf.append(",legend:!0");
		buf.append("}");
		return buf;
	}
}
