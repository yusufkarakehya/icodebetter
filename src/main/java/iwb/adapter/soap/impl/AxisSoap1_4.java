...
public class AxisSoap1_4 implements SoapAdapter{
	final static String[] elementTypes = new String[]{"","string","string","decimal","int","boolean","string","string","string"};
	final static String[] elementTypes2 = new String[]{"","string","string","decimal","string","string","string","string","string"};
	public	StringBuilder serializeSoapWSDL(W5WsServer ws, Map<String, Object> wsmoMap){
		...
		.append("<wsdl:definitions targetNamespace=\"").append(ws.getTargetNamespace()).append("\" xmlns:apachesoap=\"http://xml.apache.org/xml-soap\" xmlns:impl=\"").append(ws.getTargetNamespace()).append("\" xmlns:intf=\"").append(ws.getTargetNamespace()).append("\" xmlns:wsdl=\"http://schemas.xmlsoap.org/wsdl/\" xmlns:wsdlsoap=\"http://schemas.xmlsoap.org/wsdl/soap/\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">")
		.append("\n<wsdl:documentation>Version: 1.3</wsdl:documentation>")
		.append("\n<wsdl:types><schema elementFormDefault=\"qualified\" targetNamespace=\"").append(ws.getTargetNamespace()).append("\" xmlns=\"http://www.w3.org/2001/XMLSchema\">");
		...
		for(W5WsServerMethod wsm:ws.get_methods()){
			...
			for(W5WsServerMethodParam wsmp:wsm.get_params())if(wsmp.getOutFlag()==0 && wsmp.getParentWsMethodParamId()==0){
				buf.append("\n<element").append(wsmp.getNotNullFlag()!=0 ? "":" nillable=\"true\"").append(wsmp.getParamTip()<10 ? "":" maxOccurs=\"unbounded\"").append(" name=\"").append(wsmp.getDsc()).append("\" type=\"");
				if(wsmp.getParamTip()<9)buf.append("xsd:").append(wsmp.getNotNullFlag()==0? elementTypes2[wsmp.getParamTip()]:elementTypes[wsmp.getParamTip()]);
				else {
					...
					for(W5WsServerMethodParam swsmp:wsm.get_params())if(swsmp.getParentWsMethodParamId()==wsmp.getWsServerMethodParamId()){
						buf2.append("\n<element").append(swsmp.getNotNullFlag()!=0 ? "":" nillable=\"true\"").append(" name=\"").append(swsmp.getDsc()).append("\" type=\"");
						if(swsmp.getParamTip()<9)buf2.append("xsd:").append(swsmp.getNotNullFlag()==0? elementTypes2[swsmp.getParamTip()]:elementTypes[swsmp.getParamTip()]);
		...
		for(W5WsServerMethod wsm:ws.get_methods()){
			buf.append("\n<element name=\"").append(wsm.getDsc()).append("Response\">")
			.append("<complexType>")
			.append("<sequence>");
			for(W5WsServerMethodParam wsmp:wsm.get_params())if(wsmp.getOutFlag()!=0 && wsmp.getParentWsMethodParamId()==0){
				buf.append("\n<element").append(wsmp.getParamTip()<10 ? "":" maxOccurs=\"unbounded\"").append(" name=\"").append(wsm.getDsc()).append("Result\" type=\"");
		...
		for(W5WsServerMethod wsm:ws.get_methods()){
			buf.append("\n<wsdl:message name=\"").append(wsm.getDsc()).append("Request\">")
			.append("<wsdl:part name=\"parameters\" element=\"impl:").append(wsm.getDsc()).append("\" />")
					.append("</wsdl:message>")
					.append("<wsdl:message name=\"").append(wsm.getDsc()).append("Response\">")
					.append("<wsdl:part name=\"parameters\" element=\"impl:").append(wsm.getDsc()).append("Response\" />")
					.append("</wsdl:message>");
		}
		...
		for(W5WsServerMethod wsm:ws.get_methods()){
			buf.append("\n<wsdl:operation name=\"").append(wsm.getDsc()).append("\">")
//			.append("<wsdl:documentation xmlns:wsdl=\"http://schemas.xmlsoap.org/wsdl/\"></wsdl:documentation>")
			.append("<wsdl:input message=\"impl:").append(wsm.getDsc()).append("Request\" name=\"").append(wsm.getDsc()).append("Request\"/>")
			.append("<wsdl:output message=\"impl:").append(wsm.getDsc()).append("Response\" name=\"").append(wsm.getDsc()).append("Response\"/>")
			.append("</wsdl:operation>");
		}
		buf.append("</wsdl:portType>");
		
		buf.append("\n<wsdl:binding name=\"").append(ws.getDsc()).append("SoapBinding\" type=\"impl:").append(ws.getDsc()).append("\"><wsdlsoap:binding style=\"document\" transport=\"http://schemas.xmlsoap.org/soap/http\"/>");
		for(W5WsServerMethod wsm:ws.get_methods()){
			buf.append("\n<wsdl:operation name=\"").append(wsm.getDsc()).append("\"><wsdlsoap:operation soapAction=\"\"/>")
			.append("<wsdl:input name=\"").append(wsm.getDsc()).append("Request\"><wsdlsoap:body use=\"literal\" /></wsdl:input>")
			.append("<wsdl:output name=\"").append(wsm.getDsc()).append("Response\"><wsdlsoap:body use=\"literal\" /></wsdl:output>")
			.append("</wsdl:operation>");
		}
		buf.append("</wsdl:binding>");
		
	  
		buf.append("\n<wsdl:service name=\"").append(ws.getDsc()).append("Service\"><wsdl:port name=\"").append(ws.getDsc()).append("\" binding=\"impl:").append(ws.getDsc())
		.append("SoapBinding\"><wsdlsoap:address location=\"").append(wsSoapUrl).append(ws.getWsUrl()).append("\" /></wsdl:port>")
	...
	public	StringBuilder serializeGetFormSimple(W5WsServerMethod wsm, W5FormResult fr){
		...
		for (W5FormCellHelper fc : fr.getFormCellResults()){
			if (fc.getFormCell().getActiveFlag() != 0 && fc.getFormCell().get_sourceObjectDetail()!=null && fc.getFormCell().getControlTip() <100){
				String oo = fc.getValue();
				if(oo==null)oo="";
				buf.append("\n<").append(fc.getFormCell().getDsc()).append(">").append(GenericUtil.strToSoap(oo)).append("</").append(fc.getFormCell().getDsc()).append(">");
			}
		}
	...
	public	StringBuilder serializePostForm(W5WsServerMethod wsm, W5FormResult fr){
		...
		if(!GenericUtil.isEmpty(fr.getOutputMessages())){
			buf.append("<outMsg>").append(GenericUtil.strToSoap(fr.getOutputMessages().get(0))).append("</outMsg>");
		}
		buf.append("</").append(wsm.getDsc()).append("_result>");
		buf.append("</").append(wsm.getDsc()).append("Response>")
		.append("\n</soap:Body>")
...
//		buf.append("<data>");
		...
		if(!GenericUtil.isEmpty(qr.getData()))for(Object[] oo:qr.getData()){
			buf.append("\n<").append(wsm.getDsc()).append("Result>");
			if(wsm.get_params()==null){
				for(W5QueryField qf:qr.getQuery().get_queryFields()){
					Object oz = oo[qf.getTabOrder()-1];
					if(oz==null)oz="";
					buf.append("<").append(qf.getDsc()).append(">").append(GenericUtil.strToSoap(oz.toString())).append("</").append(qf.getDsc()).append(">");
				}
			} else {
...
//		buf.append("</data>");
	...
	public 	StringBuilder serializeDbFunc(W5WsServerMethod wsm, W5DbFuncResult dfr){
		...
		buf.append("</").append(wsm.getDsc()).append("Result></").append(wsm.getDsc()).append("Response>")//</").append(wsm.getDsc()).append("_result>
		.append("</soap:Body>")
		.append("</soap:Envelope>");
		return buf;
	}
