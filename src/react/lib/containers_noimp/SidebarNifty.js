...
function rMenu(m, p) {
  if (m.length) {
    ...
    return {
      ...
      url: p + "/" + m.id,
      icon:
        m.icon && m.icon.substr(0, 1) != "." ? "icon-" + m.icon : "icon-puzzle",
      children: n
    };
  } else {
    // node
...
function findNodes(name, currentNode) {
  ...
  if (
    currentNode.url &&
    currentNode.name &&
    currentNode.name.toLowerCase().indexOf(name) > -1
  ) {
    r.push(currentNode);
  }
  if (currentNode.children) {
    for (var qi = 0; qi < currentNode.children.length; qi++) {
      ...
      if (result !== false)
        for (var zi = 0; zi < result.length; zi++) {
          r.push(result[zi]);
        }
    }
  }
...
class Sidebar extends React.Component {
  ...
  render() {
    ...
    const wrapper = item => {
      return !item.wrapper
        ? item.name
        : _(item.wrapper.element, item.wrapper.attributes, item.name);
    };
    ...
    const divider = (divider, key) =>
      _("li", { key: key, className: "divider" });
    ...
    const navItem = (item, key) => {
      ...
      return _(
        ...
        _(
          NavLink,
          { to: item.url, className: classes, activeClassName: "active" },
          visited(item.visited),
          _("i", { className: item.icon }),
          item.name
        )
    ...
    // sidebar-nav root
    var ll = navList(this.state.items);
    ll.unshift(
      _(
        "li",
        {
          className: "nav-item menu-profile-pic",
          style: { paddingTop: 26, textAlign: "center", paddingBottom: 20 }
        },
        [
          _("img", {
            src: "sf/pic" + _scd.userId + ".png",
            className: "img-avatar",
            style: { width: 64, border: ".5px solid #cecece" },
            alt: _scd.email
          }),
          _(
            "div",
            { style: { fontWeight: "bold", paddingTop: 6 } },
            _scd.completeName
          )
        ]
      )
    ...
    return _(
      ...
      _(
        ...
        _(Nav, null /*,_('div',{className:'bg-pic'})*/, ll)
      )
    );
  }
}
