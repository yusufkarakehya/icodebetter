...
class TreeSelectBase extends Component {
    ...
    _onChange(currentNode, selectedNodes) {
            ...
            this.props.onChange && this.props.onChange(JSON.stringify(selectedNodes));
        }
        /**
...
/**
 * tested on real system with
 * , $: XDropdownMultiTreeSelect, onChange: this.onTreeSelectChange('fieldname') , defaultList:[
 */
export class XDropdownMultiTreeSelect extends TreeSelectBase {
    render() {
        ...
        return ( <
            DropdownTreeSelect data = { checkedData }
            className = { classNames('bootstrap-enbaled', 'multi-tree-select', className) }
            onChange = { this._onChange.bind(this) } {...props }
            />
        );
    }
}
...
export class XDropdownTreeSelect extends TreeSelectBase {
    _onChange(currentNode, selectedNodes) {
        // selectedNodes = this._withChildren(this._convertToArray(this.props.defaultList), selectedNodes)
        selectedNodes = this._treeFlattener(selectedNodes)
        this.props.onChange && this.props.onChange(JSON.stringify(selectedNodes[0]));
    }
    render() {
        let { className, onChange, value, defaultList, ...props } = this.props;
        let checkedData = this._prepareData(defaultList, value);
...
export class XDropdownTreeSelectResult extends TreeSelectBase {
    render() {
        let { className, onChange, value, defaultList, ...props } = this.props;
        let checkedData = this._prepareData(defaultList, value);
        return ( <
            ...
            readOnly = { true }
            disabled = { true }
            className = { classNames('bootstrap-enbaled', 'tree-select-no-edit', className) } {...props }
            />)
        }
    }